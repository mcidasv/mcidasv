...
public final class EventDispatchThreadHangMonitor extends EventQueue {
    ...
    private static final Logger logger =
        LoggerFactory.getLogger(EventDispatchThreadHangMonitor.class);
    ...
    private static class DispatchInfo {
        ...
        private void examineHang() {
            ...
            logger.debug("(hang #" + hangNumber + ") event dispatch thread stuck processing event for " + timeSoFar() + " ms:" + stackTrace);
            checkForDeadlock();
        }
        ...
        public void dispose() {
            if (lastReportedStack != null) {
                logger.debug("(hang #" + hangNumber + ") event dispatch thread unstuck after " + timeSoFar() + " ms.");
            }
        }
    }
...
//        debug("post");
    ...
    private static void checkForDeadlock() {
        ...
        logger.debug("deadlock detected involving the following threads:");
        ThreadInfo[] threadInfos = threadBean.getThreadInfo(threadIds, Integer.MAX_VALUE);
        for (ThreadInfo info : threadInfos) {
            logger.debug("Thread #" + info.getThreadId() + " " + info.getThreadName() +
    ...
    private static class Tests {
        ...
        private static void sleep(long ms) {
            try {
                logger.debug("Sleeping for " + ms + " ms on " + Thread.currentThread() + "...");
                Thread.sleep(ms);
