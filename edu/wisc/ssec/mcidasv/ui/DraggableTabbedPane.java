...
//            setUI(new CloseableTabbedPaneUI(SwingUtilities.LEFT));
    ...
    private Component originalFocusComponent;

    public boolean isEditingTab() {
        return isEditing;
    }
    
    private boolean editTabAtIndex(final int index) {
        if (isEditing) {
            return false;
        }

        if (tabEditor == null)
            tabEditor = createDefaultTabEditor();

        if (tabEditor != null) {
            prepareEditor(tabEditor, index);
            ...
            tabEditor.requestFocusInWindow();
            tabEditor.selectAll();
            return true;
        }
        return false;
    ...
    public void stopTabEdit() {
        if (editingIndex >= 0 && editingIndex < getTabCount()) {
            setTitleAt(editingIndex, oldPrefix+tabEditor.getText()+oldPostfix);
        }
    }
    ...
    public void cancelTabEditing() {
        if (tabEditor != null) {
            ...
            if (editingIndex >= 0 && editingIndex < getTabCount()) {
                Rectangle r = getBoundsAt(editingIndex);
            ...
            if (originalFocusComponent != null)
                originalFocusComponent.requestFocusInWindow();
            else
    ...
    private TabEditor createDefaultTabEditor() {
        TabEditor editor = new TabEditor();
        editor.getDocument().addDocumentListener(this);
        editor.addFocusListener(new FocusAdapter() {
            @Override public void focusGained(final FocusEvent e) {
                originalFocusComponent = e.getOppositeComponent();
            }
...
//                _tabPane.stopTabEditing();
        ...
        editor.addKeyListener(new KeyAdapter() {
            @Override public void keyPressed(final KeyEvent e) {
                if (isEditingTab() && (e.getKeyCode() == KeyEvent.VK_ESCAPE)) {
                    if (editingIndex >= 0 && editingIndex < getTabCount()) {
                        setTitleAt(editingIndex, previousValue);
                    }
                    cancelTabEditing();
                }
            }
...
//        }
        ...
        previousValue = getTitleAt(tabIndex);
        if (previousValue.startsWith("<HTML>") && previousValue.endsWith("/HTML>")) {
            oldPrefix = "<HTML>";
            oldPostfix = "</HTML>";
            String title = previousValue.substring("<HTML>".length(), previousValue.length() - "</HTML>".length());
            if (title.startsWith("<B>") && title.endsWith("/B>")) {
                title = title.substring("<B>".length(), title.length() - "</B>".length());
                oldPrefix += "<B>";
                oldPostfix = "</B>" + oldPostfix;
            }
            e.setText(title);
        }
        else {
            oldPrefix = "";
            oldPostfix = "";
            e.setText(previousValue);
        }
        e.selectAll();
        e.setForeground(getForegroundAt(tabIndex));
    }

    protected String getCurrentDisplayTitleAt(JTabbedPane tp, int index) {
        String returnTitle = tp.getTitleAt(index);
        if ((isEditing) && (index == editingIndex))
            returnTitle = tabEditor.getText();
        return returnTitle;
    }
    ...
    protected Rectangle getTabsTextBoundsAt(int tabIndex) {
        ...
        String title = getCurrentDisplayTitleAt(this, tabIndex);
...
//        Icon icon = _tabPane.getIconForTab(tabIndex);
        ...
        SwingUtilities.layoutCompoundLabel(this, getGraphics().getFontMetrics(font), title, icon,
                SwingUtilities.CENTER, SwingUtilities.CENTER,
                SwingUtilities.CENTER, SwingUtilities.TRAILING, tabRect,
                iconRect, textRect, icon == null ? 0 : textIconGap);

//        if (_tabPane.getTabPlacement() == TOP) {
            iconRect.x = tabRect.x + iconMargin;
            textRect.x = (icon != null ? iconRect.x + iconRect.width + textIconGap : tabRect.x + textPadding);
            textRect.width += 2;
//        }
//        else {
...
//        }
    ...
    protected void resizeEditor(int tabIndex) {
        ...
        tabsTextBoundsAt.x = tabsTextBoundsAt.x - tabEditor.getBorder().getBorderInsets(tabEditor).left;
        tabsTextBoundsAt.width = +tabsTextBoundsAt.width +
                tabEditor.getBorder().getBorderInsets(tabEditor).left +
                tabEditor.getBorder().getBorderInsets(tabEditor).right;
        tabsTextBoundsAt.y = tabsTextBoundsAt.y - tabEditor.getBorder().getBorderInsets(tabEditor).top;
        tabsTextBoundsAt.height = tabsTextBoundsAt.height +
                tabEditor.getBorder().getBorderInsets(tabEditor).top +
                tabEditor.getBorder().getBorderInsets(tabEditor).bottom;
        tabEditor.setBounds(SwingUtilities.convertRectangle(this, tabsTextBoundsAt, this));
