...
class Center(ImageFormatting):
    def __init__(self, lat=None, lon=None, north=None, south=None, east=None, west=None, display=None, useProjection=None):
        if lat:
            self.lat = 'lat=%s' % (lat)
        else:
            self.lat = ''
        ...
        if useProjection:
            self.useProjection = 'useProjection=%s' % (useProjection)
        else:
            self.useProjection = ''
            
    def toIsl(self):
        islString = "center %s %s %s %s %s %s %s %s" % (self.lat, self.lon, self.north, self.south, self.east, self.west, self.display, self.useProjection)
        return islString.strip() + '; '
        
class Viewpoint(ImageFormatting):
    def __init__(self, rotX=None, rotY=None, rotZ=None, transX=None, transY=None, transZ=None, scale=None, aspectX=None, aspectY=None, aspectZ=None, tilt=None, azimuth=None):
        if rotX:
            self.rotX = 'rotx=%s' % (rotX)
        else:
            self.rotX = ''
            
        if rotY:
            self.rotY = 'roty=%s' % (rotY)
        else:
            self.rotY = ''
            
        if rotZ:
            self.rotZ = 'rotz=%s' % (rotZ)
        else:
            self.rotZ = ''
            
        if transX:
            self.transX = 'transx=%s' % (transX)
        else:
            self.transX = ''
            
        if transY:
            self.transY = 'transy=%s' % (transY)
        else:
            self.transY = ''
            
        if transZ:
            self.transZ = 'transz=%s' % (transZ)
        else:
            self.transZ = ''
        ...
        if aspectX:
            self.aspectX = 'aspectx=%s' % (aspectX)
        else:
            self.aspectX = ''
            
        if aspectY:
            self.aspectY = 'aspecty=%s' % (aspectY)
        else:
            self.aspectY = ''
            
        if aspectZ:
            self.aspectZ = 'aspectz=%s' % (aspectZ)
        else:
            self.aspectZ = ''
            
        if tilt:
            self.tilt = 'tilt=%s' % (tilt)
        else:
            self.tilt = ''
            
        if azimuth:
            self.azimuth = 'azimuth=%s' % (azimuth)
        else:
            self.azimuth = ''
            
    def toIsl(self):
        islString = "viewpoint %s %s %s %s %s %s %s %s %s %s %s %s" % (self.rotX, self.rotY, self.rotZ, self.transX, self.transY, self.transZ, self.scale, self.aspectX, self.aspectY, self.aspectZ, self.tilt, self.azimuth)
        return islString.strip() + '; '
