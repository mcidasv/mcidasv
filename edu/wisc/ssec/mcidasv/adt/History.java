...
public class History {
    ...
    private static final Logger logger =
        LoggerFactory.getLogger(History.class);
    ...
    /** eye scenes */
    static String[] EyeSceneString = {
        "EYE   ", "EYE/P ", "EYE/L ", "EYE/LR", "EYE/R ", "EYE/OB"
    };
    
    /** cloud scenes */
    static String[] CloudSceneString = {
        "UNIFRM", "EMBC  ", "IRRCDO", "CRVBND", "SHEAR ", "EYE MW"
    };
    
    /** ATCF cloud */
    static String[] CloudSceneString_ATCF = {
        " CDO", "EMBC", "ICDO", "CBND", "SHER", "MEYE"
    };
    
    static String[] AutoPosStringAbbr = {
        " MAN ", "FCST ", "LAPL ", "WARM ", "SPRL ", "COMBO", "EXTRP",
        "NETCDF", " N/A "
    };
    
    static String[] Rule8String = {
        "NO LIMIT ", "0.5T/6hr ", "1.0T/6hr ", "1.7T/12hr", "2.2T/18hr",
        "2.7T/24hr", "         ", "         ", "0.2T/hour", "0.5T/hour",
        "NO LIMIT ", "0.5T/6hr ", "1.0T/6hr ", "2.7T/12hr", "3.2T/18hr",
        "3.7T/24hr", "         ", "         ", "0.2T/hour", "0.5T/hour",
        "NO LIMIT ", "0.5T/6hr ", "0.7T/6hr ", "1.2T/12hr", "1.7T/18hr",
        "2.2T/24hr", "         ", "         ", "0.2T/hour", "0.5T/hour",
        "MW Adjst "," MW ON    ", "MW ON    ", "MW HOLD  ", "MW AdjEnd"
    };
     ...
     *
     * @param OutputStyle Output one record (-1) or entire file (0).
     * @param OutputFormatTypeID 0=ATCF, -1=History List.
     * @param ATCFFileSourceIDString ATCF file source identifier.
     * @param ATCFStormIDString ATCF storm identifier.
     *
                                     ...
                                     String ATCFStormIDString)
    {
        ...
        if (OutputStyle == 0) {
            NumRecsHistory = HistoryNumberOfRecords();  // loop through history file
        } else {
            ...
            if (XInc == 0) {
                if (OutputFormatTypeID == -1) {
                    ...
                    if (UseCKZTF) {
                        ...
                        HistoryFileListing += String.format("   Date    (UTC)   CI  (DvT)/BiasAdj/(kts)  "
                            + "Tno Raw Raw    Limit  Flag Wkng  Region  Cloud  "
                            + "Type    (km)  Score   Lat     Lon    Mthd     Sat   "
                            + "VZA  Comments\n");
                    }
            ...
            if (OutputFormatTypeID == -1) {
                ...
                if(UseCKZTF) {
                    HistoryFileListing += String.format("%9s %06d  %3.1f %6.1f %5.1f  "
                        ...
                        DateString,TimeValue,CI,CIPressureValue,
                        CIWindValue,FinalT,RawTAdj,RawTOrig,
                        Rule8String[Rule8Flag],Rule9String[Rule9Flag],
                        RapidDissString[RapidDissFlag],EyeTemp,CloudTemp,
                        SceneTypeString,RadiusMaxWindString,MWScoreString,
                        LatLonComboString,
                        AutoPosStringAbbr[AutoCenteringValue],SatelliteIDString,
                        SatVZA,CommentString);
                } else {
                    HistoryFileListing += String.format("%9s %06d  %3.1f %6.1f  %+5.1f  %5.1f  "
                            + "%3.1f %3.1f %3.1f  %8s %3s  %3s  "
                            + "%6.2f %6.2f  %6s %6s %5s %15s  %5s %7s %4.1f %s\n",
                        DateString,TimeValue,CI,CIPressureValue+CIPresAdj,
                        CIPresAdj,CIWindValue,FinalT,RawTAdj,RawTOrig,
                        Rule8String[Rule8Flag],Rule9String[Rule9Flag],
                        RapidDissString[RapidDissFlag],EyeTemp,CloudTemp,
                        SceneTypeString,RadiusMaxWindString,MWScoreString,
                        LatLonComboString,
                ...
                String DateString_ATCF = String.format("%4d%02d%02d%4s",YearValue,MonthValue,DayValue,TimeString_ATCF);
                String NSString_ATCF = (Latitude>0.0) ? String.format("%s","N") : String.format("%s","S");
                ...
                HistoryFileListing += String.format("%2s, %2s, %12s, %3d, %4s, %10s, %1s, "
                        ...
                        + "%1s, %3s, %3s, %1s, "
                        + "%5s, %3s, %4s, %2d, %1d, %2d, "
                        + "%3d, %1s, %2d, %4d, %4d, %4s, %2s, "
                        + "%6s, %1s, %s\n",
                    BasinIDString_ATCF,StormIDNumString,DateString_ATCF,20,
                    "DVTO",CenterFixMethodID_ATCF," ",
                    LatitudeString,LongitudeString," ",PositionConfidence,
                    MaxWindSpeed_ATCF,WindSpeedConfidence,
                    Pressure_ATCF,PressureConfidence,"DVRK",
                    " "," "," "," "," "," "," "," "," "," "," "," "," "," ",
                    SiteID_ATCF,AutoCenterMethodString_ATCF,
                    "   I",CI_ATCF,CIConfidence,FinalTnoValue_ATCF,
                    TnoAveTimeFlag_ATCF,TnoAveTimeFlagString_ATCF,
                    RawTnoValue_ATCF,EyeTempValue,CloudTempValue,
                    SceneTypeString,Rule89_ATCF[Rule89Value],
                    SatelliteIDString,"T",CommentString);
            }
     ...
     *
     * Global structure HistoryRecordPtr will be modified and the ASCII
     * history file will be rewritten in another routine.
     *
     * @param RunFullAnalysis Full analysis toggle.
     * @param HistoryFileName File to modify.
     *
     ...
     * {@literal "type"} of modification (possible values are 1, 2, 3, 4).
     * <ol>
     *   <li>Overwritten</li>
     *   <li>Inserted</li>
     ...
     *
     * <p>Routine will modify structure HistoryRecordPtr, which will then be
     * rewritten to ASCII history file in another subroutine.</p>
     *
     * @param RunFullAnalysis Full analysis toggle.
     * @param HistoryFileName File to modify.
     *
     * @return Array containing two integer values. The first value is the
     * number of modified records, and the seconds value is the number of
     * deleted records.
     */
    public static int[] DeleteHistoryRecords(boolean RunFullAnalysis,
                                             String HistoryFileName)
    {
        int YInc = 0;
        int ModifiedCount = 0;
     ...
     *
     * @param CommentString Character string field to be inserted.
     *
     * @return If {@code -1}, there was an error finding record,
     * if {@code >= 0}, number of modified records.
     */
     ...
     * HistoryFileName}.
     *
     * @throws IOException if there was a problem writing to
     * {@code HistoryFileName}.
     */
        ...
        throws IOException
    {
        ...
        while(XInc<NumRecsHistory) {
            ...
            OutputString =
                String.format("%9s %06d %8.4f %3.1f %3.1f %3.1f %3.1f %6.2f "
                              + "%6.2f %6.2f %6.2f %6.2f %7.2f %5.1f %4.1f %4.1f "
                              + "%3d %1d %1d %2d %2d %1d %02d %1d %1d %1d %2d "
                              + "%2d %1d %2d %3d %2d %6.2f %5.1f %5.1f "
                              + "%9s %06d %4d %4d %3d %s\n",
                              DateString,
                              HistoryFile[XInc].time,
                              JulianDate,
                              HistoryFile[XInc].TrawO,
                              HistoryFile[XInc].Traw,
                              HistoryFile[XInc].Tfinal,
                              HistoryFile[XInc].CI,
                              HistoryFile[XInc].eyet,
                              HistoryFile[XInc].cloudt,
                              HistoryFile[XInc].cloudt2,
                              HistoryFile[XInc].cwcloudt,
                              HistoryFile[XInc].latitude,
                              HistoryFile[XInc].longitude,
                              HistoryFile[XInc].eyecdosize,
                              HistoryFile[XInc].eyestdv,
                              HistoryFile[XInc].cloudsymave,
                              HistoryFile[XInc].sattype,
                              HistoryFile[XInc].eyescene,
                              HistoryFile[XInc].cloudscene,
                              HistoryFile[XInc].eyesceneold,
                              HistoryFile[XInc].cloudsceneold,
                              HistoryFile[XInc].rule9,
                              HistoryFile[XInc].rule8,
                              HistoryFile[XInc].LBflag,
                              HistoryFile[XInc].rapiddiss,
                              HistoryFile[XInc].land,
                              HistoryFile[XInc].eyefft,
                              HistoryFile[XInc].cloudfft,
                              HistoryFile[XInc].ringcb,
                              HistoryFile[XInc].ringcbval,
                              HistoryFile[XInc].cwring,
                              HistoryFile[XInc].autopos,
                              HistoryFile[XInc].CIadjp,
                              HistoryFile[XInc].rmw,
                              Math.max(-99.5, HistoryFile[XInc].mwscore),
                              MWDateString,
                              HistoryFile[XInc].mwtime,
                              HistoryFile[XInc].r34,
                              HistoryFile[XInc].MSLPenv,
                              HistoryFile[XInc].vza,
                              HistoryFile[XInc].comment);
