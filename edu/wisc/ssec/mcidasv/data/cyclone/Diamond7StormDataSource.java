...
public class Diamond7StormDataSource extends StormDataSource {
	...
	public Diamond7StormDataSource(DataSourceDescriptor descriptor,
			String fileName, Hashtable properties) throws Exception {
	...
	protected void initParams() throws VisADException {
		...
		obsParams = new StormParam[] { PARAM_MAXWINDSPEED, PARAM_MINPRESSURE,
				...
				PARAM_MOVESPEED, PARAM_MOVEDIRECTION };

		forecastParams = new StormParam[] { PARAM_MAXWINDSPEED,
				PARAM_MINPRESSURE, PARAM_RADIUSMODERATEGALE,
	...
	protected void initializeStormData() {
		...
		try {
			...
			/*
			 * 
			 * diamond 7 0807 Tropical Cyclone Track Name 0807 japn 15 08 06 15
			 * 8 0 123.7 18.1 18.0 996.0 NaN NaN NaN NaN 08 06 15 14 6 124.4
			 * 18.8 19.0 995.0 NaN NaN NaN NaN year mon day time forecasttime
			 * lon lat speed pressure wind-circle-radii radii2 movspd movdir
			 */
			int lcn = 0;
			String sid = null;
			String sway = null;
			...
			while (currentIndex < lines.size()) {
				...
				while (currentIndex < endPtsIndex) {
					...
					DateTime dtt = getDateTime(yy, Integer.parseInt(mon),
							Integer.parseInt(day), Integer.parseInt(hr));
					...
					EarthLocation elt = new EarthLocationLite(new Real(
							RealType.Latitude, latitude), new Real(
							RealType.Longitude, longitude), altReal);
					...
					attributes.add(PARAM_RADIUSMODERATEGALE
							.getReal(getDouble(radiusmgale)));
					attributes.add(PARAM_RADIUSWHOLEGALE
							.getReal(getDouble(radiuswgale)));
					attributes
							.add(PARAM_MOVEDIRECTION.getReal(getDouble(mdir)));
					attributes.add(PARAM_MOVESPEED.getReal(getDouble(mspd)));

					StormTrackPoint stp = new StormTrackPoint(elt, dtt, fhour,
							attributes);
					...
					if (fhour == 0 && pts.size() > 0
							&& !trackWay.isObservation()) {
				...
				if (trackWay.isObservation()) {
					...
					sTrack = new StormTrack(sInfo, trackWay, pts, obsParams);
					stormTracks.add(sTrack);
					pts = new ArrayList();
				}
