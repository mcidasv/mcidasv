...
public abstract class StormDataSource extends DataSourceImpl {
	...
	/** _more_ */
	public static final int[] CATEGORY_VALUES = { CATEGORY_DB, CATEGORY_TD,
			CATEGORY_TS, CATEGORY_TY, CATEGORY_ST, CATEGORY_TC, CATEGORY_HU,
			CATEGORY_SD, CATEGORY_SS, CATEGORY_EX, CATEGORY_IN, CATEGORY_DS,
			CATEGORY_LO, CATEGORY_WV, CATEGORY_ET, CATEGORY_XX };

	/** _more_ */
	public static final String[] CATEGORY_NAMES = { "DB", "TD", "TS", "TY",
			"ST", "TC", "HU", "SD", "SS", "EX", "IN", "DS", "LO", "WV", "ET",
			"XX" };
	...
	protected static RealType makeRealType(String id, String alias, Unit unit) {
		try {
			alias = alias
					+ "[unit:"
					+ ((unit == null) ? "null" : DataUtil.cleanName(unit
							.toString())) + "]";
	...
	protected void initParams() throws VisADException {
		if (PARAM_STORMCATEGORY == null) {
			PARAM_STORMCATEGORY = new StormParam(Util.makeRealType(
					"stormcategory", "Storm_Category", null));
			...
			// false);
			PARAM_MAXWINDSPEED_KTS = new StormParam(makeRealType(
					"maxwindspeedkts", "Max_Windspeed", DataUtil
							.parseUnit("kts")));
	...
	protected void doMakeDataChoices() {
		...
		DataChoice choice = new DirectDataChoice(this, "stormtrack",
				"Storm Track", "Storm Track", cats, (Hashtable) null);
	...
	public StormTrackCollection getTrackCollection(StormInfo stormInfo,
			Hashtable<String, Boolean> waysToUse, Way obsWay) throws Exception {
	 ...
	 *             _more_
	 */
	public static void addDistanceError(StormTrack obsTrack, StormTrack fctTrack)
			throws VisADException {
	...
	public static StormTrack difference(StormTrack obsTrack,
			StormTrack fctTrack, StormParam param) throws VisADException,
			RemoteException {
		List<StormTrackPoint> obsTrackPoints = obsTrack.getTrackPoints();
		List<StormTrackPoint> fctTrackPoints = fctTrack.getTrackPoints();
		...
		for (StormTrackPoint forecastPoint : fctTrackPoints) {
			...
			if (range.length == 1) {
				...
				double percent = forecastDttm.getValue() - t1.getValue()
						/ (t2.getValue() - t1.getValue());

				double interpolatedValue = v2.getValue() + percent
						* (v2.getValue() - v1.getValue());
			...
			StormTrackPoint newStormTrackPoint = new StormTrackPoint(
					forecastPoint.getLocation(), forecastDttm, forecastPoint
							.getForecastHour(), new ArrayList<Real>());
	...
	public static StormTrackPoint getClosestPoint(List<StormTrackPoint> aList,
		...
		double lat = ((diff - minDiffLeft) * el1.getLatitude().getValue() + (diff - minDiffRight)
				* el2.getLatitude().getValue())
				/ diff;
		double lon = ((diff - minDiffLeft) * el1.getLongitude().getValue() + (diff - minDiffRight)
	...
	public static double getDistance(StormTrackPoint p1, StormTrackPoint p2) {
		...
		Bearing b = Bearing.calculateBearing(el1.getLatitude().getValue(), el1
