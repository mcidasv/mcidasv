...
/**
 * Created by IntelliJ IDEA. User: yuanho Date: Feb 26, 2009 Time: 10:02:45 AM
 * To change this template use File | Settings | File Templates.
 */

public class StormAODTSceneType {
	 ...
	 * @return _more_
	 */
	static float[] aodtv72_calcscene(StormAODTInfo.IRData odtcurrent,
			StormAODTInfo.DataGrid areadata)
	/*
	 * Perform Fast Fourier Transform (FFT) analysis and determine scene type
	 * via empirically defined threshold values. Inputs : global structure
	 * odtcurrent_v72 containing current intensity values Outputs : various
	 * elements within structure odtcurrent_v72 Return : -41 : error with FFT
	 * routine -51 : cloud temperature value <-100C or >+40C 0 : o.k.
	 */
	{
		...
		float radb, rade, radeye, teye, dx2, eyecnt = 0, rngcnt = 0;
		...
		/*
		 * set up arrays for FFT ananlysis. iscene=0 will perform FFT for cloud
		 * top region while iscene=1 will perform FFT for eye region
		 */
		int cursorx = areadata.numx / 2;
		int cursory = areadata.numy / 2;
		List<StormAODTInfo.RingData> tcircfirst = aodtv72_readcirc(cursorx,
				cursory, areadata);
		...
		for (iscene = 0; iscene <= 1; iscene++) {
			...
			if (iscene == 0) {
				/* CLOUD TOP */
				radb = (float) StormAODTInfo.kstart_v72;
				rade = (float) StormAODTInfo.kend_v72;
			} else {
				/* EYE REGION */
				radb = 0.0f;
				rade = (float) StormAODTInfo.kstart_v72;
			}
			...
			while (iit.hasNext()) {
				tcirc = (StormAODTInfo.RingData) iit.next();
		...
		/*
		 * compute various cloud and eye region parameters for classification
		 * scheme
		 */
		...
		radb = (float) StormAODTInfo.kstart_v72;
		rade = (float) StormAODTInfo.kend_v72;
		radeye = (float) StormAODTInfo.kstart_v72;
		Iterator it = tcircfirst.iterator();

		slice = 360.0f / (float) maxsec;
		cnteye = 0;
		while (it.hasNext()) {
			tcirc = (StormAODTInfo.RingData) it.next();
			xangle = tcirc.angle;
			xdist = tcirc.dist;
			...
			if ((xdist >= radb) && (xdist <= rade)) {
				while (ixx < maxsec) {
					...
					sectang2 = (float) Math.min(360.0, (float) (ixx + 1)
							* slice);
		...
		/*
		 * determine "minimum" temperature for each sector. This is not the
		 * actual lowest temperature, but instead is the temperature value of
		 * the coldest 90% of the values within the sector. This will,
		 * hopefully, eliminate some outliers
		 */
	...
	static float[] aodtv72_classify(StormAODTInfo.IRData odtcurrent,
			int rmwsizeman, StormAODTInfo.DataGrid areadata_v72,
			float osstr_v72, boolean osearch_v72)

	/*
	 * Classify scene type based on FFT analysis and histogram temperatures
	 * using empirically defined threshold values. Inputs : global structure
	 * odtcurrent_v72 containing current image analysis Outputs : scenetype in
	 * structure odtcurrent_v72 is modified
	 * 
	 * SCENE TYPES : EYE REGION CLOUD REGION 0 - clear 0 - uniform 1 - pinhole 1
	 * - embedded center 2 - large 2 - irregular cdo 3 - none 3 - curved band 4
	 * - shear
	 */
	{

		int iok, ixx, iyy, cloudfft = 0, cloudcat = 0, eyefft, eyecat = 0, cwtcat = 0, diffcat;
		int cbring, cbringval, diffcloudcat;
		int sceneeye, scenecloud, spiral = 0, spiralmax;
		...
		float essizeDG = 0.0f, esdiffDG = 0.0f;
		...
		float[] cdodiam = { 0.0f, 85.0f, 140.0f, 195.0f, 250.0f, 999.0f };
		float xpart, ddvor;
		int cdocat;

		boolean embdd = false, checkembc = false;
		float Ea, Eb, Ec, Ed, Ee, eyeval, Ca, Cb, Cc, Cd, Ce, cloudval;
		float diffeyecloudt, eyecloudcatdiff, eyecwtcatdiff, cloudcatdiff;
		float eyepart = 0, cloudpart = 0, cwtpart = 0, diffcat2, lastvalidtno, lastr9, maxtno;
		float[] cdomax = { 0.0f, 0.4f, 0.4f, 0.5f, 0.5f, 0.6f, 0.6f };
		int diffeyecloudcat;
		...
		for (ixx = 0; ixx < 10; ixx++) {
			...
			if ((cloudtemp <= StormAODTInfo.ebd_v72[ixx])
				...
				cloudcat = ixx;
				xpart = (float) ((cloudtemp - StormAODTInfo.ebd_v72[cloudcat]) / (StormAODTInfo.ebd_v72[cloudcat + 1] - StormAODTInfo.ebd_v72[cloudcat]));
				if (cloudcat == 0) {
					xpart = 0.0f;
				}
			...
			if ((eyetemp <= StormAODTInfo.ebd_v72[ixx])
				...
				eyecat = ixx;
				xpart = (float) ((eyetemp - StormAODTInfo.ebd_v72[eyecat]) / (StormAODTInfo.ebd_v72[eyecat + 1] - StormAODTInfo.ebd_v72[eyecat]));
				if (eyecat == 0) {
					xpart = 0.0f;
				}
			...
			if ((cloudcwt <= StormAODTInfo.ebd_v72[ixx])
					&& (cloudcwt > StormAODTInfo.ebd_v72[ixx + 1])) {
				cwtcat = ixx;
				xpart = (float) ((cloudcwt - StormAODTInfo.ebd_v72[cwtcat]) / (StormAODTInfo.ebd_v72[cwtcat + 1] - StormAODTInfo.ebd_v72[cwtcat]));
				if (cwtcat == 0) {
					xpart = 0.0f;
				}
		...
		curtime = odtcurrent.date; // aodtv72_calctime(odtcurrent.date,
									// odtcurrent.time);
		 ...
		 * else { odthistory=odthistoryfirst_v72; foundm12=FALSE; lastcscene=3;
		 * while(odthistory!=0) {
		 * xtime=aodtv72_calctime(odthistory->IR.date,odthistory->IR.time);
		 * landcheck=true;
		 * if(((oland_v72)&&(odthistory->IR.land==1))||(odthistory
		 * ->IR.Traw<1.0)) landcheck=FALSE; if((xtime<curtime)&&(landcheck)) {
		 * lastvalidtime=xtime; if((xtime>=curtimem12)&&(!foundm12)) {
		 * lasttno12=odthistory->IR.Tfinal; foundm12=true; }
		 * lasttno=odthistory->IR.Tfinal; lastcscene=odthistory->IR.cloudscene;
		 * lastescene=odthistory->IR.eyescene; if(lastescene<=2) foundeye=true;
		 * if((lastcscene==4)&&(lastescene==3)) foundeye=FALSE;
		 * lastvalidtno=lasttno; lastr9=odthistory->IR.rule9; if(lasttno>maxtno)
		 * maxtno=lasttno; } else { if(!landcheck) {
		 * 
		 * if((xtime-lastvalidtime)>0.5) { foundeye=FALSE;
		...
		/*
		 * if(eyeval>=0.00) sceneeye=5; / OBSCURED EYE / if(eyeval>=1.50)
		 * sceneeye=4; / RAGGED EYE / if(eyeval>=3.50) sceneeye=0; / CLEAR EYE /
		 */
		...
		if ((!cbscene) && (checkembc)) {
			tempval = (float) StormAODTInfo.ebd_v72[cwtcat + 1] + 273.16f;
			float[] out = aodtv72_logspiral(xlat, xlon, tempval, 1, odtcurrent,
					areadata_v72);
			spiral = (int) out[0];
		 ...
		 * ,shear,cbscene,cbgray,irrcdo);
		 */
		// (void)aodtv72_julian2cmonth(odtcurrent_v72IR.date,cdate);
		/*
		 * printf("%9s %6d %4.1f %4.1f %2d %2d %5.1f %5.1f %5.2f %5.2f %5.2f
		 * %5.2f %5.2f %5.2f %5.2f %2d %2d %4.1f %4.1f %5.2f %5.2f %5.2f %5.2f
		 * %5.2f %5.2f %6.2f %7.2f %3.1f \n",cdate,odtcurrent_v72->IR.time,
		 * cloudpart
		 * ,cwtpart,cloudfft,diffcloudcat,diffcloudt,cloudsyma,Ca,Cb,0.0
		 * ,Cc,Cd,Ce,cloudval,
		 * eyefft,diffeyecloudcat,eyepart,eyestdv,Ea,Eb,Ec,Ed
		 * ,Ee,eyeval,xlat,xlon,lasttno);
		 */
		...
		if (cbscene) {
			if (shear) {
				sceneeye = 3; /* NO EYE */
				scenecloud = 4; /* SHEAR */
				tempval = (float) StormAODTInfo.ebd_v72[3] + 273.16f;
				...
				if (cbgray) {
					...
					while ((ixx >= 2) && (!cbfound) && cont) {
						tempval = (float) StormAODTInfo.ebd_v72[ixx] + 273.16f;
				...
				if (cbfound && cont) {
					...
					/*
					 * search for maximum curved band analysis location within
					 * 1-degree box
					 */
					tempval = (float) StormAODTInfo.ebd_v72[cbring] + 273.16f;
					if (osearch_v72) {
						float[] out = aodtv72_logspiral(xlat, xlon, tempval, 2,
								odtcurrent, areadata_v72);
						spiralmax = (int) out[0];
						lat2 = out[1];
						lon2 = out[2];
					 ...
					 * ,
					 * sceneeye,diffeyecloudcat,eyefft,cwtpart,scenecloud,cloudfft
					 * ,lasttno12);
					 */
		 ...
		 * odtcurrent_v72IR.eyesceneold=-1; odtcurrent_v72IR.cloudsceneold=-1;
		 * odtcurrent_v72IR.eyecdosize=eyecdosize;
		 * odtcurrent_v72IR.ringcb=cbring; odtcurrent_v72IR.ringcbval=cbringval;
		 ...
		 */
		float[] odt = { sceneeye, scenecloud, eyecdosize, cbring, cbringval,
				cbringvalmax, cbringlatmax, cbringlonmax, odtcurrent_v72IRRMW };
	 ...
	 * @param areadata
	 *
	 * @return Array of floats that contains radius of maximum wind distance
	 * and eye size radius (km).
	 */
	static float[] aodtv72_rmw(StormAODTInfo.IRData odtcurrent,
			StormAODTInfo.DataGrid areadata)
	/*
	 * Determine radius of maximum wind based upon Jim Kossin's regression based
	 * scheme Inputs : ix0 - element location of center point iy0 - line
	 * location of center point Outputs : rmw - radius of maximum wind distance
	 * eyesize - eye size radius (km) -1 = error/eyewall not found 0 = radius
	 * found
	 */
	{
		int ixmin, ixmax, iymin, iymax;
		int ixc, iyc, i, ix, iy, idx1 = 0, idy1 = 0, idx2 = 0, idy2 = 0;
		float dx1, dx2, dy1, dy2, dav, xlat, xlon, xclat, xclon, xangle;
		float tcrit = 228.0f, warm = 223.0f;
		float rmw;
		float eyesize;

		/*
		 * calculate cursorx/cursory from numx/numy... values should be
		 * 0.5*numx/y
		 */
		ixc = areadata.numx / 2;
		iyc = areadata.numy / 2;
	 ...
	 * threshold temperature based on BD-enhancement curve values
	 *
	 ...
	 * @param areadata
	 *
	 * @return Array of floats containing: {@code bestspiral} - number of
	 * consecutive arcs through which spiral passes, {@code bestlat} - best
	 * latitude location from analysis, and {@code bestlon} - best longitude
	 * location from analysis.
	 */
	static float[] aodtv72_logspiral(float inlat, float inlon,
			float searchtemp, int searchtype, StormAODTInfo.IRData odtcurrent,
			StormAODTInfo.DataGrid areadata)
	/*
	 * Determine storm location using 10^ Log-spiral analysis. Algorithm will
	 * attempt to match the spiral with the image pixels at or below the
	 ...
	 */
	{
		...
		int ixx, iyy, izz, iskip, rotfac = 0, theta, b;
		int imaxx, iminx, imaxy, iminy, ycount;
		int spiralconsec, maxconsec, spiralbest, spiralbestrot, bestrot = 0;
		float xrad = 57.29578f, A = 25.0f, B = 10.0f / xrad;
		float maxx, minx, maxy, miny, xmindist;
		float xlat = 0.f, xlon = 0.f;
		float ylatdiff, ylondiff, xres = 6.0f;
			...
			for (iyy = iminy; iyy <= imaxy; iyy = iyy + 20) {
				...
				if (searchtype == 2) {
					...
					for (izz = 0; izz < np; izz++) {
						float[] out3 = aodtv72_distance(xlat, xlon, zlat[izz],
								zlon[izz], 1);
						if (out3[0] <= xmindist) {
							/*
				...
				if (iskip == 0) {
					/*
					 * rotate the arc spiral thru entire revolution at 30 degree
					 * intervals
					 */
					for (rotfac = 0; rotfac <= 330; rotfac = rotfac + 30) {
						...
						for (theta = 0; theta <= 540; theta = theta + 15) {
							...
							if (ycount >= 4) {
								spiralconsec++;
								/*
								 * save spiral that has the maximum consecutive
	 ...
	 * Inputs : bin - data array nbin - number of points in data array Outputs :
	 * ave - average value in array stdv - standard deviation of data array skew
	 * - skew of data array histogram
	 */
	{
		int ixx;
		...
		for (ixx = 0; ixx < nbin; ixx++) {
			...
			a3sum = a3sum
					+ ((bin[ixx] - xave) * (bin[ixx] - xave) * (bin[ixx] - xave));
		}
		if (nbin <= 1) {
			xstdv = 0.0f;
			xskew = 0.0f;
	...
	/*
	 * A Duhamel-Hollman split-radix dif fft Ref: Electronics Letters, Jan. 5,
	 * 1984 Complex input and output data in arrays x and y Length is n.
	 */
	 ...
	 * @return _more_
	 */
	static public StormAODTInfo.IRData aodtv72_gettemps(int keyer,
			StormAODTInfo.DataGrid areadata)
	/*
	 * Determine eye and coldest-warmest cloud temperatures. Inputs : keyer -
	 * eye radius (km) Outputs : odtcurrent_v72 - structure containing current
	 * image information Return : -51 : eye, cloud, or warmest temperature
	 * <-100C or >+40C
	 */
	{
		...
		StormAODTInfo.IRData odtcurrent = new StormAODTInfo.IRData();
		...
		StormAODTInfo.RingData eye = aodtv72_calceyetemp(keyer, cursortemp,
				tcircfirst);
		eyet = eye.temp;
		...
		if (keyer == StormAODTInfo.keyerA_v72) {
	 ...
	 * @return _more_
	 */
	static public StormAODTInfo.RingData aodtv72_calceyetemp(int keyer,
			float cursortemp, List<StormAODTInfo.RingData> tcircfirst)
	/*
	 * Determine eye/warmest temperature by examining the satellite data between
	 * 0 and 24/75 km from the storm center location. Eye temperature will be
	 * warmest temperature found. Inputs : keyer - analysis region radius
	 * cursortemp - temperature of pixel at cursor location Outputs : eyeangle -
	 * angle to warmest temperature in eye region (if found) eyedist - distance
	 * to warmest temperature in eye region (if found) Return : return value is
	 * warmest eye temperature
	 */
	{

		List<StormAODTInfo.RingData> tcirc;
		StormAODTInfo.RingData eye = new StormAODTInfo.RingData();
		...
		while (it.hasNext()) {
			StormAODTInfo.RingData rd = (StormAODTInfo.RingData) it.next();
	...
	static public float[] aodtv72_calccloudtemp(
			List<StormAODTInfo.RingData> tcircfirst)
	/*
	 * Determine surrounding cloud top region temperature by examining the
	 * satellite data between kstart_v72(24 km) and kend_v72 (136 km) from the
	 * storm center location. Cloud temperature will be the coldest value of an
	 * array of warmest ring temperatures (4 km ring sizes for 4 km infrared
	 * data). This is the "coldest-warmest" pixel. Inputs : none Outputs :
	 * irdist - distance (km) from center to cloud top temperature value
	 * (distance to ring) return value is cloud top temperature value
	 */
	{
		int iyy, b;
		int numrings = (StormAODTInfo.kend_v72 - StormAODTInfo.kstart_v72)
				/ StormAODTInfo.kres_v72;
		int kring;
		/* float maxtemp[200]; */
		float[] maxtemp;
		...
		while (it.hasNext()) {
			StormAODTInfo.RingData rd = (StormAODTInfo.RingData) it.next();
			if ((rd.dist >= (float) StormAODTInfo.kstart_v72)
					&& (rd.dist < (float) StormAODTInfo.kend_v72)) {
				kring = ((int) rd.dist - StormAODTInfo.kstart_v72)
						/ StormAODTInfo.kres_v72;
		...
		for (iyy = 0; iyy < numrings; iyy++) {
			if ((maxtemp[iyy] < cloudtemp) && (maxtemp[iyy] > 160.0)) {
				...
				irdist = (iyy * StormAODTInfo.kres_v72)
						+ StormAODTInfo.kstart_v72;
			}
		}
	 ...
	 * @return _more_
	 */
	static public List<StormAODTInfo.RingData> aodtv72_readcirc(int ixc,
			int iyc, StormAODTInfo.DataGrid areadata)
	/*
	 * Read array of satellite data temperatures and load array containing
	 * temperatures and lat/lon positions. Inputs : ixc - element location of
	 * center point iyc - line location of center point Outputs : global
	 * structure tcirc containing temperature/locations
	 */
	{
		int ixx, iyy, b, count = 0;
		float xclat, xclon, xlat, xlon, xdist, xangle;
		StormAODTInfo sinfo = new StormAODTInfo();
		StormAODTInfo.RingData tcirc = null;

		List<StormAODTInfo.RingData> tcircfirst = new ArrayList();
		...
		for (iyy = 0; iyy < areadata.numy; iyy++) {
			for (ixx = 0; ixx < areadata.numx; ixx++) {
				...
				xdist = outDA[0];
				xangle = outDA[1];
				if (xdist <= (float) (StormAODTInfo.kend_v72 + 80)) { /*
																	 * add 80.0
																	 * to allow
																	 * for
																	 * correct
																	 ...
																	 */
					count++;
					tcirc = new StormAODTInfo.RingData(xdist, xangle,
							areadata.temp[iyy][ixx]);
					tcircfirst.add(tcirc);
					// tcirc.nextrec=NULL; /* make pointer for last record equal
					// to 0 */
				}
			}
		}
	...
	/*
	 * Calculate distance and angle between two points (rrlat,rrlon and
	 * pplat,pplon). Inputs : rrlat - latitude of starting point rrlon -
	 * longitude of starting point rrlat - latitude of ending point rrlon -
	 * longitude of ending point iunit - flag for output unit type
	 * (1-km,2-mi,3-nmi) Outputs : dist - distance between two points ang -
	 * angle between two points (0=north)
	 */
	{
		...
		if (iunit == 3) {
			xdist = ((60.0f * xdist) + 55f) / 111.0f; /*
													 * conversion to nautical
													 * miles
													 */
		}
		dist = xdist;
		...
		if (Math.abs(xdist) > 0.0001) {
			xang = (float) ((Math.sin(rlon - plon) * Math.sin((3.14159 / 2.0)
					- plat)) / Math.sin(idist));
		}
	 ...
	 * latitude/longitude and distance/angle values. Inputs : rlat - initial
	 * latitude rlon - initial longitude dist - distance from initial position
	 * ang - angle from initial position Outputs : plat - derived latitude plon
	 * - derived longitude
	 */
	{
		...
		qlat = (float) Math.acos((Math.cos(clat) * Math.cos(cdis))
				+ (Math.sin(clat) * Math.sin(cdis) * Math.cos(cang)));
	...
	/*
	 * Compute time in xxxxx.yyy units, where xxxxx is the day and yyy is the
	 * percentage of the day. This routine will also correct for Y2K problems.
	 * Inputs : date - Julian date time - time in HHMMSS format Outputs :
	 ...
	 */
	{
		...
		/*
		 * check for millenium designation in the year. if it is not there, add
	 ...
	 *
	 * @return Array of floats containing: {@code valb} - eye/cdo radius
	 * or shear distance, and {@code valc} - eye/cdo symmetry value or 0.
	 */
	static float[] aodtv72_cdoshearcalc(float xlat, float xlon, float tempval,
			int atype, StormAODTInfo.DataGrid areadata)
	/*
	 * Determine eye size or shear distance for a given scene. Inputs : xlat -
	 * center latitude of analysis grid xlon - center longitude of analysis grid
	 * tempval - temperature threshold value to be used atype - analysis type
	 * (1-cdo size,2-eye size,3-shear distance) Outputs : valb - eye/cdo radius
	 * or shear distance valc - eye/cdo symmetry value or 0
	 */
	{

		int ixx, iyy, np, b, numvalid = 0;
		float smalldist, vc, maxdist;
		float[] zlat, zlon;
		float a1, a2, a3, a4, v1, v2, v3;
		float valb = 0.f, valc = 0.f;
		/* allocate memory */
		...
		if (atype == 1) {
			...
			if (np < (areadata.numx * areadata.numy)) {
				for (ixx = 0; ixx < np; ixx++) {
					...
					if (out[0] > StormAODTInfo.keyerM_v72) {
						if ((Math.abs(out[1] - 45.0) <= 15.0) && (out[0] < a1)) {
							a1 = out[0];
						}
						if ((Math.abs(out[1] - 135.0) <= 15.0) && (out[0] < a2)) {
							a2 = out[0];
						}
						if ((Math.abs(out[1] - 225.0) <= 15.0) && (out[0] < a3)) {
							a3 = out[0];
						}
						if ((Math.abs(out[1] - 315.0) <= 15.0) && (out[0] < a4)) {
							a4 = out[0];
						}
					}
				}
				...
				numvalid = 4;
				v3 = StormAODTInfo.keyerM_v72 + StormAODTInfo.kres_v72;
				if (a1 < v3) {
					numvalid--;
				}
				if (a2 < v3) {
