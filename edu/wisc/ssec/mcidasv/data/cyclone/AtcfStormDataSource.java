...
public class AtcfStormDataSource extends StormDataSource {
	...
	protected void initParams() throws VisADException {
		...
		if (obsParams == null) {
			obsParams = new StormParam[] { PARAM_STORMCATEGORY,
					PARAM_MINPRESSURE, PARAM_MAXWINDSPEED_KTS };
	...
	private boolean readTracks(StormInfo stormInfo,
			...
			Hashtable<String, Boolean> waysToUse, boolean throwError)
			throws Exception {
		...
		if (isZip) {
			GZIPInputStream zin = new GZIPInputStream(new ByteArrayInputStream(
					bytes));
		...
		for (int i = 0; i < lines.size(); i++) {
			...
			int category = ((IDX_TY < toks.size()) ? getCategory((String) toks
					.get(IDX_TY)) : CATEGORY_XX);
			...
			if (track == null) {
				...
				track = new StormTrack(stormInfo, addWay(way), new DateTime(
						dttm), obsParams);
			...
			double windspeed = ((IDX_VMAX < toks.size()) ? getDouble((String) toks
					.get(IDX_VMAX))
					: Double.NaN);
			double pressure = ((IDX_MSLP < toks.size()) ? getDouble((String) toks
					.get(IDX_MSLP))
					: Double.NaN);
	...
	public StormTrackCollection getTrackCollectionInner(StormInfo stormInfo,
		...
		int nowYear = new GregorianCalendar(DateUtil.TIMEZONE_GMT)
				.get(Calendar.YEAR);
		int stormYear = getYear(stormInfo.getStartTime());
		// If its the current year then its in the aid_public dir
		String aSubDir = ((stormYear == nowYear) ? "aid_public"
				: ("archive/" + stormYear));
		String bSubDir = ((stormYear == nowYear) ? "btk"
				: ("archive/" + stormYear));
		if (!justObs) {
		...
		trackFile = getFullPath(bSubDir + "/" + PREFIX_BEST
				+ stormInfo.getBasin().toLowerCase() + stormInfo.getNumber()
				+ stormYear + ".dat.gz");
		try {
