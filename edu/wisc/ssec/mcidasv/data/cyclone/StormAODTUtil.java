...
/**
 * Created by IntelliJ IDEA. User: yuanho Date: Mar 9, 2009 Time: 2:16:30 PM To
...
public class StormAODTUtil {
	...
	public static String aodtv72_textscreenoutput(
			StormAODTInfo.IRData odtcurrent, DecimalFormat latLonFormat)
	/*
	 * Output information to text screen. Inputs : global structure
	 * odtcurrent_v72 containting current image information Outputs : none
	 */
	{
		...
		char iout2;
		String latc = "N", lonc = "W";
		String[] cr9 = { "OFF   ", "ON    ", "WEAKEN" };
		String[] crpd = { "OFF   ", "FLAG  ", "ON    ", "ON    " };
		String[] cbd = { "LOW CLD", "OFF WHT", "DK GRAY", "MD GRAY", "LT GRAY",
				"BLACK  ", "WHITE  " };
		String[] cr8 = { "NO LIMIT ", "0.5T/6hr ", "1.2T/6hr ", "1.7T/12hr",
				"2.2T/18hr", "2.7T/24hr", "         ", "         ",
				"0.1T/hour", "0.5T/hour", "NO LIMIT ", "0.5T/6hr ",
				"1.7T/6hr ", "2.2T/12hr", "2.7T/18hr", "3.2T/24hr",
				"         ", "         ", "0.1T/hour", "0.5T/hour",
		...
		String[] basin = { "ATLANTIC    ", "WEST PACIFIC", "EAST PACIFIC",
				"INDIAN      " };
		String[] cuse = { "MANUAL", "FORECAST INTERPOLATION",
				"LAPLACIAN ANALYSIS", "WARMEST PIXEL SEARCH",
				"SPIRAL ANALYSIS", "RING/SPIRAL COMBINATION",
		...
		boolean bettercb = false, rmwflag = false;
		...
		if (cloudcat == 2) {
			scenetype = StormAODTInfo.cloudtype_v72[cloudcat];
		} else if (cloudcat == 3) {
			...
			if (eyecat <= 2) {
				scenetype = StormAODTInfo.eyetype_v72[eyecat];
				if (eyecat <= 2) {
					rmwflag = true;
				}
				...
				scenetype = "CLOUD REGION* "
						+ StormAODTInfo.cloudtype_v72[cloudcat];
			}
		...
		result.append(HtmlUtil.row(label("Time:") + HtmlUtil.col(ctime)
				+ label("Lat:")
				+ HtmlUtil.col(latLonFormat.format(odtcurrent.latitude))
				+ label("Lon:")
				+ HtmlUtil.col(latLonFormat.format(odtcurrent.longitude))));
		result.append("</table>");
		result.append("<table>");
		result.append(HtmlUtil.row(HtmlUtil.cols("<b>CI#</b>",
				"<b>Pressure</b>", "<b>Vmax</b>")));
		result.append(HtmlUtil.row(HtmlUtil.cols("" + odtcurrent.CI, ""
		...
		result.append("</table>");
		result.append("<table>");
		result.append(HtmlUtil.row(label("Center Temp:")
				+ HtmlUtil.col("" + odtcurrent.eyet)));
		result.append(HtmlUtil.row(label("Cloud Region Temp:")
				+ HtmlUtil.col("" + cloudtemp)));
		result.append(HtmlUtil.row(label("Scene Type:")
				+ HtmlUtil.col(scenetype)));
		result.append("</table>");
	...
	/*
	 * Convert degree.degree to degree/minute/second format. Inputs : llval -
	 * latitude/longitude to convert Outputs : degree - degrees minute - minutes
	 * second - seconds
	 */
	{
		int deg;
	...
	/*
	 * Convert yyyyddd to dd/mm/yy format. Inputs : syd - Julian day (yyyyddd)
	 * Outputs : day - date month - month year - year (yyyy)
	 */
	{
		DateTime dt = null;
	 ...
	 * @return _more_
	 */
	public static float aodtv72_getpwval(int ival, float cival, int idomain_v72)
	/*
	 * Obtain pressure or wind_v72 value (for Atlantic or West Pacific storms)
	 * given the intensity estimate value. Inputs : ival - flag for wind_v72 (1)
	 * or pressure (0) output cival - Current Intensity (CI) value Outputs :
	 * return value is pressure/wind_v72 value
	 */
	{
		float value;
		int ixx = 2;
		...
		while ((cival > StormAODTInfo.tno_v72[ixx]) && (ixx < 82)) {
			ixx++;
		}

		/* convert CI value to wind_v72/pressure value */
		if (ival == 1) {
			value = (float) StormAODTInfo.wind_v72[ixx]; /* WIND */
		} else {
			value = (float) StormAODTInfo.pres_v72[idomain_v72][ixx]; /* PRESSURE */
		}
