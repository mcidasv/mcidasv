...
public class McvPluginManager extends PluginManager {
    ...
    private Set<Plugin> myPlugins = new LinkedHashSet<Plugin>();

    /** List of plugins that are not local */
    private Set<Plugin> otherPlugins = new LinkedHashSet<Plugin>();
...
//        ((edu.wisc.ssec.mcidasv.ResourceManager)getResourceManager()).writePluginIndex(Plugin.getPlugins());
    ...
    public void updatePlugins() {
        ...
        for (Plugin plugin : Plugin.plugins) {
            ...
            String sizeString = "";
            if (plugin.size != null && plugin.size.length() > 0) {
    ...
    private static class Plugin {
        ...
        public static List<List<String>> getPlugins() {
            List<List<String>> pluginList = new ArrayList<List<String>>(plugins.size());
            for (Plugin p : plugins) {
                List<String> asList = new ArrayList<String>(6);
                asList.add(p.name);
                asList.add(p.category);
                asList.add(p.desc);
                asList.add(p.url);
                asList.add(p.size);
                asList.add(Double.toString(p.version));
                pluginList.add(asList);
            }
            return pluginList;
        }
