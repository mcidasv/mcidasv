...
/**
 * A MetApps Display Control with Displayable and controls for displaying a
 * track (balloon sounding or aircraft track)
 * 
...
public class StormTrackControl extends DisplayControlImpl {
	...
	/** _more_ */
	private final static String PREF_STORMDISPLAYSTATE = "pref.stormtrackcontrol.stormdisplaystate";
	...
	/** _more_ */
	final ImageIcon ICON_ON = GuiUtils
			.getImageIcon("/ucar/unidata/idv/control/storm/dot.gif");

	/** _more_ */
	final ImageIcon ICON_OFF = GuiUtils
			.getImageIcon("/ucar/unidata/idv/control/storm/blank.gif");
	...
	/** _more_ */
	private Hashtable<StormInfo, StormDisplayState> stormDisplayStateMap = new Hashtable<StormInfo, StormDisplayState>();
	...
	public boolean init(DataChoice dataChoice) throws VisADException,
		...
		DataChoice.addCurrentName(new TwoFacedObject(
				"Storm Track>Forecast Hour", "fhour"));
		DataChoice.addCurrentName(new TwoFacedObject(
				"Storm Track>Forecast Time", "rhour"));
		DataChoice.addCurrentName(new TwoFacedObject(
				"Storm Track>Forecast STI Time", "shour"));
		...
		if (okWays == null) {
			okWays = (Hashtable<String, Boolean>) getPreferences().get(
					getPref(PREF_OKWAYS));
		}
		...
		if (okParams == null) {
			okParams = (Hashtable<String, Boolean>) getPreferences().get(
	...
	private JComponent getWaysToUseComp() {
		...
		if (waysToUsePreferenceCbx == null) {
			waysToUsePreferenceCbx = new JCheckBox("Save as preference", false);
		}
		waysToUseSelector = new TwoListPanel(allWays, "Don't Use", useWays,
				"Use", null, false);
		JComponent contents = GuiUtils.centerBottom(waysToUseSelector, GuiUtils
				.left(waysToUsePreferenceCbx));
	...
	private boolean applyWaysToUse() {
		...
		if (!useWays.equals(only)) {
			...
			if (only.size() == allWays.size()) {
				onlyShowTheseWays(new ArrayList<Way>(), waysToUsePreferenceCbx
						.isSelected());
			} else {
				onlyShowTheseWays((List<Way>) only, waysToUsePreferenceCbx
						.isSelected());
			}
		}
		return changed;
	...
	protected void addPropertiesComponents(JTabbedPane jtp) {
		...
		if (stormDataSource.getIsObservationWayChangeable()) {
			...
			for (Way way : allWays) {
				...
				JRadioButton jrb = new JRadioButton(way.getId(), Misc.equals(
						observationWay, way));
			...
			JComponent obsWayContents = GuiUtils.topLeft(GuiUtils.doLayout(
					obsWayRadioButtons, ((obsWayRadioButtons.size() > 10) ? 2
							: 1), GuiUtils.WT_N, GuiUtils.WT_N));
			int width = 200;
			int height = 150;
			if (obsWayRadioButtons.size() > 10) {
				obsWayContents = GuiUtils.makeScrollPane(obsWayContents, width,
						height);
			}
			jtp.add("Observation " + getWayName(), GuiUtils.centerBottom(
	 ...
	 * if(chartParamsSelector!= null) { List pa =
	 * chartParamsSelector.getCurrentEntries(); return pa; } return null; }
	 */
	...
	public void viewpointChanged() {
		...
		synchronized (MUTEX) {
			StormDisplayState sds = getCurrentStormDisplayState();
			HashMap<Way, List> wayToTracksMap = sds.getTrackCollection()
					.getWayToTracksHashMap();
			...
			/*
			 * if ( !getHaveInitialized() || !getActive()) { return; }
			 * Rectangle2D newBounds = calculateRectangle(); boolean
			 * shouldReload = false; if ((lastViewBounds == null) ||
			 * (lastViewBounds.getWidth() == 0) || (lastViewBounds.getHeight()
			 * == 0)) { shouldReload = true; } else if (
			 * !(newBounds.equals(lastViewBounds))) { double widthratio =
			 * newBounds.getWidth() / lastViewBounds.getWidth(); double
			 * heightratio = newBounds.getHeight() / lastViewBounds.getHeight();
			 * double xdiff = Math.abs(newBounds.getX() -
			 * lastViewBounds.getX()); double ydiff = Math.abs(newBounds.getY()
			 * - lastViewBounds.getY()); // See if this is 20% greater or
			 * smaller than before. if ((((widthratio < .80) || (widthratio >
			 * 1.20)) && ((heightratio < .80) || (heightratio > 1.20))) ||
			 * ((xdiff > .2 * lastViewBounds.getWidth()) || (ydiff > .2 *
			 * lastViewBounds.getHeight()))) { shouldReload = true; } } float
			 * newScale = getScaleFromDisplayable(); if
			 * (Float.floatToIntBits(lastViewScale) !=
			 * Float.floatToIntBits(newScale)) { shouldReload = true; } if
			 * (shouldReload) {
			 * 
			 * updateLayoutModel();
			 * 
	...
	protected FieldImpl makeTrackField(StormTrack track, StormParam param)
		...
		Real[] values = ((param == null) ? null : track
				.getTrackAttributeValues(param));
		Unit unit = ((param != null) ? param.getUnit() : null);
		for (int pointIdx = 0; pointIdx < numPoints; pointIdx++) {
			StormTrackPoint stp = points.get(pointIdx);
			Real value = ((values == null) ? null : values[pointIdx]);
			...
			if (rangeType == null) {
				...
				if (rangeType == null) {
					cnt++;
					if (track.getWay().toString()
							.startsWith("Observation_year"))
						rangeType = Util.makeRealType("trackrange_"
								+ track.getWay() + "_" + cnt, unit);
					else
						rangeType = Util.makeRealType("trackrange_"
								+ track.getId() + "_" + track.getWay() + "_"
								+ cnt, unit);
			...
			alts[pointIdx] = 1;
			// if(Math.abs(lats[i])>90) System.err.println("bad lat:" +
			// lats[i]);
		}
		GriddedSet llaSet = ucar.visad.Util
				.makeEarthDomainSet(lats, lons, alts);
		Set[] rangeSets = new Set[] { new DoubleSet(new SetType(rangeType)) };
		FunctionType newType = new FunctionType(((SetType) llaSet.getType())
				.getDomain(), rangeType);
		FlatField trackField = new FlatField(newType, llaSet,
				(CoordinateSystem) null, rangeSets, new Unit[] { unit });
	 ...
	 *             _more_
	 */
	public void setColorTable(String whichColorTable, ColorTable newColorTable)
	...
	protected void getSaveMenuItems(List items, boolean forMenuBar) {
		StormDisplayState current = getCurrentStormDisplayState();
		if ((current != null) && current.getActive()) {
			items.add(GuiUtils.makeMenuItem("Save Storm Display as Preference",
					this, "saveStormDisplayState"));
	...
	protected void getEditMenuItems(List items, boolean forMenuBar) {
		items.add(MenuUtil.makeCheckboxMenuItem("Edit Mode", this, "editMode",
				null));

		StormDisplayState current = getCurrentStormDisplayState();
		if ((current != null) && current.getActive()) {
			items.add(GuiUtils.makeMenuItem("Add Forecast Time Chart", current,
					"addForecastTimeChart"));
			items.add(GuiUtils.makeMenuItem("Add Forecast Hour Chart", current,
		...
		try {
			...
			for (int i = stormInfos.size() - 1; i >= 0; i--) {
				...
				if (stormDisplayState.getActive()) {
					activeItems.add(MenuUtil.makeMenuItem(stormInfo.toString(),
				...
				yearMenu.add(GuiUtils.makeMenuItem(stormInfo.toString(), this,
						"viewStorm", stormDisplayState));
			}
			...
			if (activeItems.size() > 0) {
				...
				trackMenu.insert(GuiUtils
						.makeMenu("Active Tracks", activeItems), 0);
			}
	...
	private Hashtable getPreferences() {
		if (preferences == null) {
			...
			preferences = (Hashtable) getIdv().getStore().getEncodedFile(path);
			if (preferences == null) {
	...
	private StormDisplayState getStormDisplayState(StormInfo stormInfo) {
		StormDisplayState stormDisplayState = stormDisplayStateMap
				.get(stormInfo);
		try {
			if (stormDisplayState == null) {
				...
				if (template != null) {
					try {
						stormDisplayState = (StormDisplayState) getIdv()
								.decodeObject(template);
	...
	public void initDone() {
		super.initDone();
		try {
			for (Enumeration keys = stormDisplayStateMap.keys(); keys
					.hasMoreElements();) {
				...
				if (mapProjection != null) {
					...
					if (mvm != null) {
						mvm.setMapProjection(mapProjection, true,
		...
		getControlContext().getStationModelManager().addPropertyChangeListener(
				this);
	}
	...
	private void loadYearPointData() {
		try {
			if (yearLabels == null) {
				...
				StationModelManager smm = getControlContext()
			...
			if (allPointObs.size() == 0) {
				...
				yearLabels.setStationData(PointObFactory
						.makeTimeSequenceOfPointObs(allPointObs, -1, -1));
			}
		} catch (Exception exc) {
			logException("Loading year", exc);
		}
	...
	public void loadYearInner(YearDisplayState yds) throws Exception {
		...
		for (int i = stormInfos.size() - 1; i >= 0; i--) {
			...
			if (obsTrack == null) {
				...
				try {
					StormTrackCollection tracks = stormDataSource
							.getTrackCollection(stormInfo, obsWays,
									observationWay);
					...
					if (errorWindow == null) {
						...
						errorWindow.getContentPane().add(
...
//						errorWindow.show();
			...
			if (obsTrack != null) {
				...
				times.add(dttm);
				fields.add(field);
				Tuple tuple = new Tuple(new Data[] { new visad.Text(textType,
						stormInfo.toString()) });
	...
	public void writeToKml() {
		...
		JComponent accessory = GuiUtils.top(GuiUtils.vbox(obsCbx, forecastCbx,
				mostRecentCbx));

		String filename = FileManager.getWriteFile(Misc
				.newList(FileManager.FILTER_KML), FileManager.SUFFIX_KML,
				accessory);
		if (filename == null) {
			return;
		}
		...
		try {
			writeToKml(filename, obsCbx.isSelected(), forecastCbx.isSelected(),
	 ...
	 *             _more_
	 */
	public void writeToKml(String filename, boolean doObs, boolean doForecast,
			boolean mostRecent) throws VisADException, RemoteException {
		try {
			Element kmlNode = KmlUtil.kml("");
			Element docNode = KmlUtil.document(kmlNode, "");
			KmlUtil
					.iconstyle(docNode, "hurricaneicon",
							"http://www.unidata.ucar.edu/software/idv/kml/images/hurricane.png");
			Hashtable state = new Hashtable();
			for (StormDisplayState stormDisplayState : getActiveStorms()) {
				stormDisplayState.writeToKml(docNode, state, doObs, doForecast,
						mostRecent);
			}
			...
			for (YearDisplayState yds : ydss) {
				...
				Element yearNode = KmlUtil.folder(docNode, "Year:"
						+ yds.getYear());
			...
			IOUtil.writeBytes(new File(filename), XmlUtil.toString(kmlNode)
					.getBytes());
	 ...
	 *             _more_
	 */
	protected void writeToGE(Element docNode, Hashtable state, Element parent,
			StormTrack track, Color color) throws Exception {
		Element placemark = KmlUtil.placemark(parent, "Track", "<html>"
				+ getWayName() + ":" + track.getWay() + "<br>" + ""
				+ track.getStartTime() + "</html>");

		int cnt = 0;
		String dateString = track.getStartTime().formattedString(
				"yyyy-MM-dd hhmm", DateUtil.TIMEZONE_GMT);
		...
		for (StormTrackPoint stp : track.getTrackPoints()) {
			...
			if (track.getWay().isObservation()) {
				Element icon = KmlUtil.placemark(parent, "Time:"
						+ stp.getTime(),
						"<html><table>" + formatStormTrackPoint(track, stp)
								+ "</table></html>", el.getLatitude().getValue(
								visad.CommonUnit.degree), el.getLongitude()
								.getValue(visad.CommonUnit.degree), (el
								.getAltitude() != null ? el.getAltitude()
								.getValue() : 0), "#hurricaneicon");
				KmlUtil
						.timestamp(icon, ucar.visad.Util
								.makeDate(stp.getTime()));
			}
		...
		if (state.get(styleUrl) == null) {
			Element style = KmlUtil.linestyle(docNode, styleUrl, color, track
					.getWay().isObservation() ? 3 : 2);
			state.put(styleUrl, style);
		}
		KmlUtil.styleurl(placemark, "#" + styleUrl);
		Element linestring = KmlUtil.linestring(placemark, false, false, sb
				.toString());
		// KmlUtil.timestamp(linestring, track.getStartTime());
		if (!track.getWay().isObservation()) {
			KmlUtil.timestamp(placemark, ucar.visad.Util.makeDate(track
					.getStartTime()));
		} else {
		}
	}
	...
	protected Container doMakeContents() throws VisADException, RemoteException {
		...
		stormInfos = (List<StormInfo>) Misc.sort(stormDataSource
				.getStormInfos());

		if (stormInfos.size() == 1) {
			try {
				if (localStormDisplayState == null) {
					localStormDisplayState = new StormDisplayState(stormInfos
							.get(0));
				}
				stormDisplayStateMap = new Hashtable<StormInfo, StormDisplayState>();
		...
		if (yearPanels.size() > 0) {
			int width = 300;
			int height = 400;
			JScrollPane scroller = GuiUtils.makeScrollPane(GuiUtils
					.top(yearComponent), width, height);
		...
		JComponent yearTopComp = GuiUtils.inset(GuiUtils.left(GuiUtils.label(
				"Time Mode: ", timeModeBox)), 5);

		treePanel.addComponent(GuiUtils.topCenter(yearTopComp, yearComponent),
				null, "Yearly Tracks", null);
		...
		for (int i = stormInfos.size() - 1; i >= 0; i--) {
			...
			treePanel.addComponent(panelContents, category, stormInfo
					.toString(), stormDisplayState.getActive() ? ICON_ON
					: ICON_OFF);
		...
		if ((startTime != null) && (endTime != null)) {
			try {
				...
				for (StormInfo stormInfo : stormInfos) {
					double date = Util.makeDate(stormInfo.getStartTime())
							.getTime();
					StormDisplayState stormDisplayState = getStormDisplayState(stormInfo);
	...
	public void stormChanged(StormDisplayState stormDisplayState) {
		...
		if (treePanel != null) {
			treePanel.setIcon(stormDisplayState.getContents(),
					stormDisplayState.getActive() ? ICON_ON : ICON_OFF);
		}
	}
	 ...
	 * 
	 * @param value
	 *            The new value for StormDisplayStates
	 */
	...
	public List<StormDisplayState> getStormDisplayStates() {
		List<StormDisplayState> stormDisplayStates = new ArrayList<StormDisplayState>();
		for (Enumeration keys = stormDisplayStateMap.keys(); keys
				.hasMoreElements();) {
			StormInfo key = (StormInfo) keys.nextElement();
			StormDisplayState stormDisplayState = stormDisplayStateMap.get(key);
			// TODO: We don't want to add every state, just the ones that have
			// been changed
	...
	public YearDisplayState getYearDisplayState(int year) {
		YearDisplayState yearDisplayState = yearDisplayStateMap
				.get(new Integer(year));
	 ...
	 * 
	 * @param value
	 *            The new value for YearDisplayStates
	 */
	...
	public List<YearDisplayState> getYearDisplayStates() {
		List<YearDisplayState> yearDisplayStates = new ArrayList<YearDisplayState>();
		for (Enumeration keys = yearDisplayStateMap.keys(); keys
				.hasMoreElements();) {
	...
	protected List getCursorReadoutInner(EarthLocation el, Real animationValue,
			int animationStep, List<ReadoutInfo> samples) throws Exception {
	 ...
	 *            _more_
	 * @return A 2-tuple. First element is the StormTrack. Second element is the
	 *         ob. Or null if none found
	 * 
	 * @throws Exception
	 *             _more_
	 */
	protected Object[] findClosestPoint(EarthLocation el,
			List<StormDisplayState> theStates, Real animationValue,
			int distanceThresholdPixels) throws Exception {
		...
		// System.err.println ("click:" + clickPt[0]+"/"+clickPt[1] + " "
		// +minDistance);
		...
		for (int i = 0; i < numStates; i++) {
			...
			HashMap<Way, List> wayToTracksMap = trackCollection
					.getWayToTracksHashMap();
			// Way obsWay = new Way(Way.OBSERVATION);
			java.util.Set<Way> ways = wayToTracksMap.keySet();
			...
			for (Way way : ways) {
				...
				if (way.equals(Way.OBSERVATION)) {
					// WayDisplayState trackWDS = wayToTracksMap.get(way);
					// //get(Way.OBSERVATION);
				...
				// System.err.println(way + " track time is: " +
				// track.getStartTime());
			...
			// System.err.println ("\t" + obScreen[0]+"/"+obScreen[1] + " d:" +
			// distance);
	...
	private boolean checkTracksVisible(Real currentAnimationTime,
			WayDisplayState wds) throws Exception {
		...
		if (wds.shouldShowTrack() && wds.hasTrackDisplay()) {
			...
			if (timeSet.getLength() == 1) {
				...
				float timeValueFloat = (float) currentAnimationTime
						.getValue(timeSet.getSetUnits()[0]);
	...
	private StormTrack getClosestTimeForecastTrack(List<StormTrack> tracks,
			Real pTime) throws VisADException {
	 ...
	 * 
	 * @param value
	 *            The new value for LocalStormDisplayState
	 */
	 ...
	 * 
	 * @param value
	 *            The new value for YearTimeMode
	 */
