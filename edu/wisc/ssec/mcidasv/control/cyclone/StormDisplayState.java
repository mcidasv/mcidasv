...
public class StormDisplayState {
	...
	private static Color[] colors = { Color.RED, Color.PINK, Color.MAGENTA,
			Color.ORANGE, Color.YELLOW, Color.GREEN, Color.BLUE, Color.CYAN,
		...
		for (WayDisplayState wds : wayDisplayStates) {
			...
			if (wds.getWay().isObservation()) {
				...
				wds.getWayState().setBackground(
						forecastState.getWayState().getVisible() ? null
								: bgcolor);
				wds.getRingsState()
						.setBackground(
								(rowOk && forecastState.getRingsState()
										.getVisible()) ? null : bgcolor);
				wds.getConeState()
						.setBackground(
								(rowOk && forecastState.getConeState()
										.getVisible()) ? null : bgcolor);
				wds.getTrackState()
						.setBackground(
								(rowOk && forecastState.getTrackState()
										.getVisible()) ? null : bgcolor);
			}
		}
	}
	...
	private void handleMouseDrag(DisplayEvent event, EarthLocation newPt)
		...
		double deltaX = LatLonPointImpl.lonNormal(oldPt.getLongitude()
				.getValue(CommonUnit.degree))
				- LatLonPointImpl.lonNormal(newPt.getLongitude().getValue(
						CommonUnit.degree));
		...
		if ((event.getModifiers() & event.CTRL_MASK) != 0) {
			...
			for (int i = stretchIndex - 1; i >= 0; i--) {
				...
				EarthLocation newEl = makePoint(pt.getLatitude().getValue(
						CommonUnit.degree)
						- deltaY * percent, LatLonPointImpl.lonNormal(pt
						.getLongitude().getValue(CommonUnit.degree))
						- deltaX * percent);
				// System.err.println("   " +percent + " " + pt.getLatLonPoint()
				// + " " + newEl.getLatLonPoint());
			...
			for (StormTrackPoint stp : points) {
				...
				EarthLocation newEl = makePoint(pt.getLatitude().getValue(
						CommonUnit.degree)
						- deltaY, LatLonPointImpl.lonNormal(pt.getLongitude()
						.getValue(CommonUnit.degree))
						- deltaX);
		...
		getCommandManager().add(
				new PointEditCommand(editedStormTrack, originalPoints,
						editedStormTrack.getTrackPoints()));
	...
	protected List<WayDisplayState> getWayDisplayStates() {
		return (List<WayDisplayState>) Misc.toList(wayDisplayStateMap
				.elements());
	}
	...
	protected WayDisplayState getWayDisplayState(Way way) {
		...
		if (wayState == null) {
			wayDisplayStateMap.put(way, wayState = new WayDisplayState(this,
					way));
	...
	private JComponent makeList(List stormParams, final Object id) {
		...
		list
				.setToolTipText("<html>Parameter used for cone<br>Control-click for multiple select</html>");
...
//		}
	...
	private void initCenterContents() {
		...
		mainContents.removeAll();
		JButton unloadBtn = GuiUtils.makeImageButton(
				"/auxdata/ui/icons/Cut16.gif", this, "deactivate");
		...
		JComponent top = GuiUtils.inset(GuiUtils.leftRight(GuiUtils
				.lLabel(label), unloadBtn), new Insets(0, 0, 0, 0));
		...
		if (forecastRadiusParams != null) {
			...
			if (params.get(ID_FORECAST_CONE) == null) {
				params.put(ID_FORECAST_CONE, Misc.newList(forecastRadiusParams
						.get(0)));
			}
		}
		...
		JComponent obsLayoutComp = new LayoutModelWidget(stormTrackControl,
				this, "setObsLayoutModel", getObsLayoutModel(), true);
		JComponent obsPointLayoutComp = new LayoutModelWidget(
				stormTrackControl, this, "setObsPointLayoutModel",
				getObsPointLayoutModel(), true);
		JComponent forecastLayoutComp = new LayoutModelWidget(
				stormTrackControl, this, "setForecastLayoutModel",
		...
		JComponent obsConeComp = ((obsRadiusParams != null) ? makeList(
				obsRadiusParams, ID_OBS_CONE) : (JComponent) GuiUtils.filler());
		JComponent obsRingComp = ((obsRadiusParams != null) ? makeBox(
				obsRadiusParams, ID_OBS_RINGS,
				"Parameter used for observation rings") : (JComponent) GuiUtils
				.filler());

		JComponent forecastConeComp = ((forecastRadiusParams != null) ? makeList(
				forecastRadiusParams, ID_FORECAST_CONE)
				: (JComponent) GuiUtils.filler());
		JComponent forecastRingComp = ((forecastRadiusParams != null) ? makeBox(
		...
		JComponent forecastModeComp = GuiUtils.inset(GuiUtils.left(GuiUtils
				.label("Animation Mode: ", timeModeBox)), 5);
		...
		topComps
				.add(GuiUtils.vbox(forecastColorByBox, forecastColorTableLabel));
		...
		for (Way way : ways) {
			...
			JComponent labelComp = GuiUtils.hbox(wds.getWayState()
					.getCheckBox(), new JLabel(" " + way.toString()));
			...
			if (way.isObservation()) {
				...
				comps.add(col++, GuiUtils.wrap(wds.getTrackState()
						.getCheckBox()));
				if (obsRadiusParams != null) {
					comps.add(col++, GuiUtils.wrap(wds.getRingsState()
							.getCheckBox()));
					comps.add(col++, GuiUtils.wrap(wds.getConeState()
							.getCheckBox()));
				}
				...
				if (!haveDoneForecast) {
					...
					comps.add(GuiUtils.filler());
					comps.add(GuiUtils.hbox(forecastState.getWayState()
							.getCheckBox(), GuiUtils
							.lLabel("<html><u><i>Forecasts:</i></u></html>")));
					comps.add(GuiUtils.wrap(forecastState.getTrackState()
							.getCheckBox()));
					if (forecastRadiusParams != null) {
						comps.add(GuiUtils.wrap(forecastState.getRingsState()
								.getCheckBox()));

						comps.add(GuiUtils.wrap(forecastState.getConeState()
								.getCheckBox()));
					}
				}
				comps.add(swatch);
		...
		for (int colIdx = 0; colIdx < numCols; colIdx++) {
			...
			if (s.length() > 0) {
				comps.add(colIdx, new JLabel("<html><u><i>" + s
						+ "</i></u></html>"));
			} else {
		...
		JComponent wayComp = GuiUtils.topLeft(GuiUtils.doLayout(comps, numCols,
		...
		wayComp = GuiUtils.left(GuiUtils.doLayout(new Component[] {
				GuiUtils.left(paramComp), GuiUtils.filler(2, 10),
				GuiUtils.left(wayComp) }, 1, GuiUtils.WT_N, GuiUtils.WT_NNY));
		...
		if (charts.size() == 0) {
			charts.add(new StormTrackChart(this, "Storm Chart",
					StormTrackChart.MODE_FORECASTTIME));
		}
		for (StormTrackChart stormTrackChart : charts) {
			tabbedPane.addTab(stormTrackChart.getName(), stormTrackChart
					.getContents());
		}
...
//							displayMaster.setActive(true);
	...
	private void showStormInner() throws Exception {
		...
		if (trackCollection == null) {
			if (mainContents != null) {
				mainContents.removeAll();
				mainContents.add(GuiUtils.top(GuiUtils.inset(new JLabel(
						"Loading Tracks..."), 5)));
				mainContents.invalidate();
				mainContents.validate();
				mainContents.repaint();
			...
			trackCollection = stormTrackControl.getStormDataSource()
					.getTrackCollection(stormInfo,
							stormTrackControl.getOkWays(),
							stormTrackControl.getObservationWay());
			initCenterContents();
			stormTrackControl
					.addDisplayable(holder = new CompositeDisplayable());
			// Add the tracks
			for (StormTrack track : trackCollection.getTracks()) {
				WayDisplayState wayDisplayState = getWayDisplayState(track
						.getWay());
		...
		if (wds != null) {
			...
			for (StormTrackTableModel trackModel : tableModels) {
				if (trackModel.getStormTrack().equals(track)) {
					...
					Component comp = (Component) track
							.getTemporaryProperty(PROP_TRACK_TABLE);
...
//					displayMaster.setActive(true);
		...
		if (obsColorTableLabel != null) {
			...
			obsColorTableLabel.setIcon(((ct != null) ? ColorTableCanvas
					.getIcon(ct) : null));
		...
		if (forecastColorTableLabel != null) {
			...
			forecastColorTableLabel.setIcon(((ct != null) ? ColorTableCanvas
					.getIcon(ct) : null));
	...
	protected String getColorTableToolTip(boolean forObs) {
		...
		Range range = getStormTrackControl().getIdv().getParamDefaultsEditor()
				.getParamRange(param.getName());
		if (range == null) {
			return "Color table preview";
		}

		Unit displayUnit = getStormTrackControl().getIdv()
				.getParamDefaultsEditor().getParamDisplayUnit(param.getName());
	...
	protected StationModel getObsLayoutModel() {
		if ((obsLayoutModelName == null) || obsLayoutModelName.equals("none")) {
			return null;
		}

		StationModelManager smm = stormTrackControl.getControlContext()
				.getStationModelManager();
		return smm.getStationModel(obsLayoutModelName);
		/*
		 * StationModel model = new StationModel("TrackLocation"); ShapeSymbol
		 * shapeSymbol = new ShapeSymbol(0, 0);
		 * shapeSymbol.setShape(ucar.visad.ShapeUtility.HURRICANE);
		 * shapeSymbol.setScale(2.0f); shapeSymbol.bounds = new
		 * java.awt.Rectangle(-15, -15, 30, 30);
		 * shapeSymbol.setRectPoint(Glyph.PT_MM);
		 * shapeSymbol.setForeground(null); model.addSymbol(shapeSymbol); return
		 * model;
		 */
	}
	...
	/**
	 * Animation animation = stormTrackControl.getViewAnimation(); if (animation
	 * == null) { return; } List<StormTrack> visibleTracks = new
	 * ArrayList<StormTrack>(); Real currentAnimationTime =
	 * animation.getAniValue(); if (currentAnimationTime == null ||
	 * currentAnimationTime.isMissing()) { return; } Iterate way display states
	 * boolean visible = false; if(wds.shouldShowTrack() &&
	 * wds.hasTrackDisplay()) { FieldImpl field =
	 * (FieldImplt)wds.getTrackDisplay().getData() if(field==null) continue; Set
	 * timeSet = GridUtil.getTimeSet(); if(timeSet == null) continue; if
	 * (timeSet.getLength() == 1) { visible = true; } else { //Else work the
	 * visad magic float timeValueFloat = (float) currentAnimationTime.getValue(
	 * timeSet.getSetUnits()[0]); // System.err.println("multiple times:" +
	 * timeValueFloat); float[][] value = { { timeValueFloat } }; int[] index =
	 * timeSet.valueToIndex(value); // System.err.println("index:" + index[0]);
	 * visible = (index[0] >= 0); } if(visible) { //Find the closest track in
	 * wds in time visibleTracks.add(..); } }
	 * 
	 * 
	 * Now search in space
	 * 
	 * @param stormTrackChart
	 *            _more_
	 */
	...
	private JComponent getTrackTable() {
		...
		for (StormTrack track : trackCollection.getTracks()) {
			...
			if (!track.getWay().isObservation()) {
				contents = GuiUtils.topCenter(GuiUtils.left(GuiUtils.inset(
						new JLabel(track.getStartTime().toString()), 5)),
						contents);
			}
			...
			contents = GuiUtils.centerBottom(contents, GuiUtils
					.right(flythroughBtn));
			tableTreePanel.addComponent(contents, track.getWay().toString(),
					track.getStartTime().toString(), null, track);
		}
	...
	public void flythroughTrack(StormTrack track) {
		try {
			...
			for (StormTrackPoint stp : track.getTrackPoints()) {
				EarthLocation newLoc = makePoint(stp.getLocation()
						.getLatitude().getValue(CommonUnit.degree), stp
						.getLocation().getLongitude().getValue(
								CommonUnit.degree));
	...
	/** _more_ */
	public static final PatternFileFilter FILTER_DAT = new PatternFileFilter(
			".+\\.dat", "Diamond Format (*.dat)", ".dat");
	...
	public void writeToDataFile() {
		try {
			JComponent accessory = GuiUtils.top(GuiUtils.vbox(obsCbx,
					forecastCbx, mostRecentCbx, editedCbx));

			String filename = FileManager.getWriteFile(Misc.newList(
					FileManager.FILTER_XLS, FILTER_DAT),
			...
			if (forecastCbx.isSelected()) {
				...
				for (Way way : waysToUse) {
					...
					List<StormTrack> tracks = (List<StormTrack>) Misc
							.sort(trackMap.get(way));
	...
	public void writeToKml(Element docNode, Hashtable state, boolean doObs,
			boolean doForecast, boolean mostRecent) throws VisADException,
			RemoteException {
		try {
			List<Way> waysToUse = new ArrayList<Way>();
			Hashtable<Way, List> trackMap = new Hashtable<Way, List>();
			...
			Element topFolder = KmlUtil.folder(docNode, "Storm: "
					+ stormInfo.toString()
					+ "   "
					+ stormInfo.getStartTime().formattedString("yyyy-MM-dd",
							DateUtil.TIMEZONE_GMT));
			...
			if ((obsTrack != null) && doObs) {
				...
				stormTrackControl.writeToGE(docNode, state, obsFolder,
						obsTrack, getWayDisplayState(obsTrack.getWay())
								.getColor());
			}
			if (doForecast) {
				waysToUse = Misc.sort(waysToUse);
				for (Way way : waysToUse) {
					...
					Element wayNode = KmlUtil.folder(topFolder,
							stormTrackControl.getWayName() + ": " + way);
					List<StormTrack> tracks = (List<StormTrack>) Misc
							.sort(trackMap.get(way));
					if (mostRecent) {
						StormTrack recent = tracks.get(tracks.size() - 1);
						stormTrackControl.writeToGE(docNode, state, wayNode,
								recent, getWayDisplayState(recent.getWay())
										.getColor());
					} else {
						for (StormTrack track : tracks) {
							stormTrackControl.writeToGE(docNode, state,
									wayNode, track, getWayDisplayState(
	 ...
	 * 
	 * @param value
	 *            The new value for WayDisplayStateMap
	 */
	 ...
	 * 
	 * @param value
	 *            The new value for ForecastState
	 */
	 ...
	 * 
	 * @param value
	 *            The new value for ObsLayoutModelName
	 */
	 ...
	 * 
	 * @param value
	 *            The new value for ForecastLayoutModelName
	 */
