...
public class WayDisplayState {
	...
	public void updateDisplay(boolean force) throws Exception {
		...
		int forecastAnimationMode = stormDisplayState
		...
		if (shouldShowTrack()) {
			...
			if (force || !hadTrack || paramChanged || modeChanged
					|| stormDisplayState.isColorRangeChanged()) {
				...
				if (trackField != null) {
					...
					if (colorParam != null) {
						...
						range = stormDisplayState.getStormTrackControl()
						...
						if (stormDisplayState.isColorRangeChanged()) {
							...
							stormDisplayState.getStormTrackControl()
									.getColorTableWidget(range);
						}

						Unit displayUnit = stormDisplayState
								.getStormTrackControl().getIdv()
								.getParamDefaultsEditor().getParamDisplayUnit(
										paramName);
						if (displayUnit != null) {
	...
	public void updateLayoutModel() throws Exception {
		StationModel sm;
		// If we are showing the track then create (if needed) the station model
		// displays
		if (shouldShowTrack()) {
			if (way.isObservation()) {
				sm = stormDisplayState.getObsPointLayoutModel();
				// We won't create them (or will remove them) if the layout
				// model is null
				if (sm == null) {
					...
					if (true) { // (!hasObsPointDisplay()) {
						...
						pointField = PointObFactory.makeTimeSequenceOfPointObs(
								allPointObs, -1, -1);
					...
					if (hasObsPointDisplay()) { // && !Misc.equals(sm,
												// getObsPointDisplay().getStationModel()))
												// {
			...
			sm = (way.isObservation() ? stormDisplayState.getObsLayoutModel()
					: stormDisplayState.getForecastLayoutModel());
			if (sm == null) {
				removeLabelDisplay();
			} else {
				if (pointObs.size() > 0) { // (!hasLabelDisplay()) {
					FieldImpl pointField = PointObFactory
							.makeTimeSequenceOfPointObs(pointObs, -1, -1);
	...
	/**
	 * Declutters the observations. This is just a wrapper around the real
	 * decluttering in doTheActualDecluttering(FieldImpl) to handle the case
	 * where there is a time sequence of observations.
	 * 
	 * @param obs
	 *            initial field of observations.
	 * @param sModel
	 *            _more_
	 * 
	 ...
	 * 
	 * @param pointObs
	 *            point observations for one timestep.
	 * 
	...
	private FieldImpl doTheActualDecluttering(FieldImpl pointObs,
		...
		Rectangle2D scaledGlyphBounds = new Rectangle2D.Double(glyphBounds
				.getX()
				* myScale, glyphBounds.getY() * myScale, glyphBounds.getWidth()
				* myScale, glyphBounds.getHeight() * myScale);
		NavigatedDisplay navDisplay = stormDisplayState.getStormTrackControl()
				.getNavigatedDisplay();
		...
		if (v.isEmpty()) {
			retField = new FieldImpl((FunctionType) pointObs.getType(),
					new Integer1DSet(((SetType) domainSet.getType())
							.getDomain(), 1));
	...
	protected Rectangle2D calculateRectangle() {
		try {
			Rectangle2D.Double box = stormDisplayState.getStormTrackControl()
					.getNavigatedDisplay().getVisadBox();
			if (!box.isEmpty()) {
				// pad rectangle by 5%
	...
	public StationModelDisplayable getLabelDisplay() throws Exception {
		if (labelDisplay == null) {
			StationModel sm = (way.isObservation() ? stormDisplayState
					.getObsLayoutModel() : stormDisplayState
			...
			if (sm != null) {
				...
				labelDisplay.setScale(stormDisplayState.getStormTrackControl()
						.getDisplayScale());
			}
		}
		return labelDisplay;
	...
	public StationModelDisplayable getObsPointDisplay() throws VisADException,
		...
		if (obsPointDisplay == null) {
			...
			obsPointDisplay.setScale(stormDisplayState.getStormTrackControl()
					.getDisplayScale());
		}
	...
	public void initTrackDisplay() throws Exception {
		...
		trackDisplay = new TrackDisplayable("track_"
				+ stormDisplayState.getStormInfo().getStormId()); // +
																	// stormDisplayState.getColorParam(this));
		if (way.isObservation()) {
	...
	public TrackDisplayable makeRingDisplay(StormParam param, int mode)
		...
		TrackDisplayable ringDisplay = new TrackDisplayable("ring_"
				+ stormDisplayState.getStormInfo().getStormId());
	...
	protected JComponent getColorSwatch() {
		if (colorSwatch == null) {
			XmlObjectStore store = stormDisplayState.getStormTrackControl().getIdv().getStore();
	...
	private void setTrackColor() throws Exception {
		if (trackDisplay != null) {
			if (colorParam == null
					|| colorParam.getName().equalsIgnoreCase("Fixed")) {
	...
	private void setLabelColor() throws Exception {
		Color c = getColor();
		if (labelDisplay != null) { // && !Misc.equals(c,
									// labelDisplay.getColor())) {
			labelDisplay.setColor(c);
		}
		if (obsPointDisplay != null) { // && !Misc.equals(c,
										// obsPointDisplay.getColor())) {
	...
	protected FieldImpl makeTrackField(int mode) throws Exception {
		...
		// Use a local list to hold the point obs so we don't run into a race
		// condition
		...
		for (StormTrack track : tracks) {
			FieldImpl field = stormDisplayState.getStormTrackControl()
					.makeTrackField(track, colorParam);
			if (field == null) {
				continue;
			}
			if (i == 0)
	 ...
	 *             _more_
	 */
	public void addDisplayable(Displayable displayable) throws VisADException,
			RemoteException {
	 ...
	 * 
	 * @param value
	 *            The new value for StormDisplayState
	 */
	...
	public FieldImpl makeRingTracks(StormTrack track, StormParam param)
		...
		for (StormTrack ringTrack : stracks) {
			datas[i++] = stormDisplayState.getStormTrackControl()
					.makeTrackField(ringTrack, null);
		}
		return Util.indexedField(datas, false);
	}
	...
	public StormTrackPoint getCirclePoint(StormTrackPoint stp, double r0,
			double azimuth, DateTime dt) throws VisADException {
		//
		...
		EarthLocation el1 = new EarthLocationLite(lp.getLatitude(), lp
				.getLongitude(), 0);
	...
	public StormTrack makeConeTrack_Old(StormTrack track, StormParam param)
		...
		StormTrackPoint[] halfCircle = getHalfCircleTrackPoint(lastEl, ang,
				((r != null) ? r.getValue() : 0), last.getTime());
	...
	public StormTrack makeConeTrack(StormTrack track, StormParam param)
		...
		List<StormTrackPoint> conePointsLeft = new ArrayList<StormTrackPoint>();
		List<StormTrackPoint> conePointsRight = new ArrayList<StormTrackPoint>();
		...
		List<StormTrackPoint> p2c = getPointToCircleTangencyPointA(stp1, stp2,
				stp3, param, true);
		while (p2c == null) { // need to find the first point with param value
			...
			p2c = getPointToCircleTangencyPointA(stp1, stp2, stp3, param, true);
			nn++;
			if (nn >= size) {
				break;
			}
		...
		if (p2c != null) {
			...
			p2c = getPointToCircleTangencyPointA(stp1, stp2, stp3, param, false);
		...
		for (int i = nn; i < size; i++) {
			...
			if (p2c != null) {
				...
				p2c = getCircleToCircleTangencyPointA(stp1, stp2, stp3, param,
						false);
				conePointsLeft.addAll(p2c);
				stp1 = stp2; // update the first point only after the valid
								// second point
			}
		...
		EarthLocation lastEl = last.getLocation();
		StormTrackPoint endSTP = conePointsRight
				.get(conePointsRight.size() - 1);
		/*
		 * int ii = 0; while ((endSTP == null) && (ii < (size - 2))) { ii++;
		 * last = stps.get(size - 1 - ii); lastEl = last.getLocation(); endSTP =
		 * conePointsRight.get(size - 1 - ii); }
		 * 
		 * if ((endSTP == null) || (ii == (size - 2))) { return null; }
		 */
		if (endSTP == null) {
			return null;
		}
	...
	public Bearing getStormPoinsBearing(StormTrackPoint sp1, StormTrackPoint sp2) {
		...
		return Bearing.calculateBearing(el1.getLatitude().getValue(), el1
				.getLongitude().getValue(), el2.getLatitude().getValue(), el2
				.getLongitude().getValue(), null);
	...
	public List<StormTrackPoint> getPointToCircleTangencyPointA(
		...
		for (int i = 0; i < n; i++) {
			...
			EarthLocation el = new EarthLocationLite(lp1.getLatitude(), lp1
					.getLongitude(), 0);
	...
	public StormTrackPoint[] getHalfCircleTrackPoint(EarthLocation c,
		...
		for (int i = 0; i < size; i++) {
			...
			EarthLocation el = new EarthLocationLite(lp.getLatitude(), lp
					.getLongitude(), 0);
	...
	public StormTrackPoint[] getHalfCircleTrackPointOld(EarthLocation c,
		...
		ProjectionPointImpl p0 = e.latLonToProj(c.getLatitude().getValue(), c
		...
		for (int i = 0; i < size; i++) {
			...
			EarthLocation el = new EarthLocationLite(lp11.getLatitude(), lp11
					.getLongitude(), 0);
			StormTrackPoint sp = new StormTrackPoint(el, dt, 0, null);
