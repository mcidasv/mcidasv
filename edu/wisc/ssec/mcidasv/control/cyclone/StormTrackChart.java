...
public class StormTrackChart {
	...
	private JComponent doMakeContents() {
		...
		JComponent chartComp = GuiUtils.leftCenter(chartLeft, GuiUtils
				.topCenter(chartTop, getChart().getContents()));
		return chartComp;
	}
	...
	public TimeSeriesChart getChart() {
		if (timeSeries == null) {
			timeSeries = new TimeSeriesChart(stormDisplayState
					.getStormTrackControl(), "Track Charts") {
	...
	private List<Integer> findForecastHours() {
		...
		for (StormTrack track : stormDisplayState.getTrackCollection()
			...
			for (StormTrackPoint stormTrackPoint : track.getTrackPoints()) {
				Integer forecastHour = new Integer(stormTrackPoint
						.getForecastHour());
	...
	protected void createChart() {
		...
		List<Way> ways = Misc.sort(stormDisplayState.getTrackCollection()
				.getWayList());
		...
		for (Way way : ways) {
			...
			final JCheckBox cbx = new JCheckBox(way.toString(), chartWays
					.contains(theWay));
		...
		chartComps.add(new JLabel(stormDisplayState.getStormTrackControl()
				.getWaysName()
				+ ":"));
		...
		chartComps.add(GuiUtils.lLabel((isHourly() ? "Forecast Hour:"
				: "Forecast Time:")));
	...
	protected List getStormTrackParams() {
		...
		if ((params == null) || (params.size() == 0)) {
			...
			if (params.size() == 0) {
				StormTrack obsTrack = stormDisplayState.getTrackCollection()
						.getObsTrack();
				if (obsTrack != null) {
	...
	protected LineState makeLine(StormTrack stormTrack, StormParam param) {
		...
		// StormDataSource.TYPE_STORMCATEGORY);
		...
		lineState.setColor(stormDisplayState.getWayDisplayState(
		...
		try {
				...
				for (Way way : chartWays) {
					...
					for (Integer fHour : forecastHours) {
						...
						for (StormTrack track : tracksFromWay) {
							StormTrackPoint stp = track
									.findPointWithForecastHour(fHour.intValue());
							if (stp != null) {
						...
						if (points.size() > 0) {
							...
							tracksToUse.add(new StormTrack(stormDisplayState
									.getStormInfo(), way, points, null));
						}
					}
				}
			...
			for (StormParam param : chartParams) {
				...
				for (StormTrack track : tracksToUse) {
					...
					if (lineState.getRange().getMin() == lineState.getRange()
							.getMin()) {
	 ...
	 * 
	 * @param value
	 *            The new value for ChartForecastTime
	 */
	 ...
	 * 
	 * @param value
	 *            The new value for ChartDifference
	 */
