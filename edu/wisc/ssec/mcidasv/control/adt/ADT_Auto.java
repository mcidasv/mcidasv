...
public class ADT_Auto {
   ...
   public static double[] AutoMode2(double InputLatitudePosition, double InputLongitudePosition) throws IOException {
      ...
      /* NEED TO MULTIPLY LONGITUDES BY -1.0 SINCE ADT ROUTINES WERE DEVELOPED ON McIDAS-X,
      ...
      /* need to flip final Longitude value back to McV format by multiplying by -1.0 */
   ...
   private static double[] SpiralCenterLowRes(double InputLatitude, double InputLongitude) {
      ...
      for(YInc=0;YInc<IRData_NormCoord_NumberRows-1;YInc++) {
         for(XInc=0;XInc<IRData_NormCoord_NumberColumns-1;XInc++) {
            ...
            NSTempGradientArray[YInc][XInc] = GradientLOGReduction*NSTempGradientArray[YInc][XInc];
            EWTempGradientArray[YInc][XInc] = GradientLOGReduction*EWTempGradientArray[YInc][XInc];
         }
      }
      ...
      for(YInc=1;YInc<=(int)SpiralCenterAnalysisField[0][0];YInc++) {
         ...
         SpiralCenterAnalysisField[YInc][2] = ((LongitudeMultFactor*SpiralCenterAnalysisField[YInc][2])/
                                               (Math.cos(PI*InputLatitude/180.0)))+InputLongitude;
      }
   ...
   private static void Gradient(double TemperatureInputArray[][], int ElementXNumber, int LineYNumber,
                                double LongitudeIncrement, double LatitudeIncrement) {
   ...
   private static int[] Lalo2IndsFloat( double LatitudeInput, double LongitudeInput,
                                        double LatitudeArrayInput[][], double LongitudeArrayInput[][],
                                        int ElementXNumber, int LineYNumber) {
      ...
      int YAxisPositionReturn = (int)((((double)LineYNumber-1.0)/(LatitudeMaximum-LatitudeMinimum))*
                                        (LatitudeMaximum-LatitudeInput));
      int XAxisPositionReturn = (int)((((double)ElementXNumber-1.0)/(LongitudeMaximum-LongitudeMinimum))*
                                        (LongitudeMaximum-LongitudeInput));
   ...
   private static double[] Inds2LaloFloat(int XAxisPosition, int YAxisPosition,
      ...
      double LongitudeReturn = LongitudeMaximum-((((double)XAxisPosition)/((double)ElementXNumber-1.0))*
                                                   (LongitudeMaximum-LongitudeMinimum));
      double LatitudeReturn = LatitudeMaximum-((((double)YAxisPosition)/((double)LineYNumber-1.0))*
   ...
   private static void CircleFilt(int RingRadiusInput) {
      ...
      double ThresholdDifference=0.5*(double)(((RingRadiusInput+1)*(RingRadiusInput+1))-
                                               (RingRadiusInput*RingRadiusInput));
   ...
   private static void MoatMaskCalc(double MoatMaskTempThreshold, double RingFitMaxRadiusDegree, int MoatSignCheckFlag) {
      ...
      for(ArrayInc=1;ArrayInc<=NumberOfObjects;ArrayInc++) {
         ...
         double FeatureLength = Math.sqrt(Math.pow((LatitudeMaximum-LatitudeMinimum),2)+
                                Math.pow((LongitudeMaximum-LongitudeMinimum)/Math.cos((PI*AverageLatitude)/180.0),2));
      ...
      for(XInc=0;XInc<IRData_Remap_NumberColumns;XInc++) {
         for(YInc=0;YInc<IRData_Remap_NumberRows;YInc++) {
            if(MoatSignCheckFlag==1) {
               BlackWhiteFieldArray[YInc][XInc] = (IRData_Remap_Temperature[YInc][XInc]> TemperatureThresholdValue) ? 1 : 0;
            } else {
               BlackWhiteFieldArray[YInc][XInc] = (IRData_Remap_Temperature[YInc][XInc]< TemperatureThresholdValue) ? 1 : 0;
            }
         }
    ...
    */
      ...
      double LatitudeIncrement = Math.abs(IRData_Remap_Latitude[0][0]-IRData_Remap_Latitude[1][0])/2.0;
      double LongitudeIncrement = Math.abs(IRData_Remap_Longitude[0][1]-IRData_Remap_Longitude[0][0])/2.0;
    ...
    */
      ...
      if((ADT_Main.HistoryFileName!=null)||(ForceAutoFixUseTF_forTesting)) {
         ...
         if(((CurEyeScene<3)||((CurCloudScene==1)&&(CurEyeScene==3)))&&(EyeSceneCount==2)) {
            FoundEyeSceneTF = true;
         }
      }
      ...
      return new double[] { FinalLatitude, FinalLongitude, FinalScoreValue, (double)PositioningMethodID};
    ...
    */
      ...
      remap_vars.out_bfw = Math.max(MINBFW,Math.max(LineSplineInput,MINBLKSIZ)*tiff_vars.out_elems);
    
      remap_vars.slb = remap_vars.in_bfw/tiff_vars.in_elems;
      remap_vars.dlb = ((remap_vars.out_bfw/tiff_vars.out_elems)/LineSplineInput)*LineSplineInput;
    ...
    */
      ...
      if(FoundPointTF) {
         ...
         XElementValueReturn = ((IndexValue==0)||(IndexValue==2)) ? ElementValue : ElementValue+1;
            ...
            for(XInc=1;XInc<remap_vars.ncl+1;XInc++) {
               ...
               if(LineCoordinateArray[ArrayPointValue-1]==(double)-99.9) {
                  ...
                  for(IncVal=0;IncVal<8;IncVal++) {
                     ...
                     if(!SKIP) {
                        if((LineCoordinateArray[ValueIND1-1]==-99.9)||(LineCoordinateArray[ValueIND2-1]==-99.9)) { 
                           SKIP = true;
                        }
            ...
            for(SourceBlockInc=1;SourceBlockInc<tiff_vars.in_lines+1;SourceBlockInc=SourceBlockInc+remap_vars.slb) {
               ...
               while(FirstCornerPointer<LastCornerPointer) {
                  for(SplineInc=1;SplineInc<remap_vars.nspl+1;SplineInc++) {
                     LABEL30 = false;
                     OffsetValue0 = (SourceBlockValue/remap_vars.nspl)*LineSplineInput*tiff_vars.out_elems;
                     OffsetValue1 = (SourceBlockValue%remap_vars.nspl)*ElementSplineInput;
                     ...
                     if(!LABEL30) { 
                        ...
                        LineValueC = (LineLRValue+LineULValue-LineURValue-LineLLValue)/SplinePixelValue;
                        ElementValueA = (ElementURValue-ElementULValue)/ElementSplineInput;
                        ElementValueB = (ElementLLValue-ElementULValue)/LineSplineInput;
                        ElementValueC = (ElementLRValue+ElementULValue-ElementLLValue-ElementURValue)/SplinePixelValue;
                        ...
                        if((SplineInc==remap_vars.nspl)||(ElementMinimumFinal<1||(ElementMaximumFinal>tiff_vars.in_elems))||
                          ...
                          ((FirstCornerPointer+(2*remap_vars.ncl)-1)>LastCornerPointer)) {
                           for(YInc=1;YInc<LineSplineInput+1;YInc++) {
                              ...
                              for(XInc=1;XInc<ElementSplineInput+1;XInc++) {
                                 ...
                                 if(!LABEL38) SourceElement = (int)(ElementValueBCounter+ElementValueACounter0);
                                 if(SourceElement<1) {
                                    LABEL38 = true;
                                 }
                                 if((SourceElement>tiff_vars.in_elems)&&(EdgeFixID==0)) {
                                    LABEL38 = true;
                                 }
                                 if(!LABEL38) {
                                    if(SourceElement>tiff_vars.in_elems) {
                                       SourceElement = SourceElement-tiff_vars.in_elems;
                                    }
                                    SourceOffsetValue = ((SourceLine-SourceBlockInc)*tiff_vars.in_elems)+SourceElement;
                                    OffsetValueE = OffsetValue1+XInc;
                                    if(OffsetValueE>tiff_vars.out_elems) {
                                       LABEL38 = true;
                                    }
                                    if(!LABEL38) OffsetValueL = OffsetValue0+ElementMiscValue;
                                    if((OffsetValueL/(tiff_vars.out_elems+AccumulatedLines-1))>tiff_vars.out_lines) {
                                       LABEL38 = true;
                                    }
                                    if(!LABEL38) {
