...
public class ReprojectSwath {
 ...
 public void projectSwathToGrid(int trackStart, int trackStop) throws VisADException, RemoteException {
    ...
    int[] swathIdx = new int[1];
    
    for (int j=trackStart; j <= trackStop; j++) {
       for (int i=0; i < xtrackLen; i++) {
         swathIdx[0] = j*xtrackLen + i;

         float[][] swathCoord = swathDomain.indexToValue(swathIdx);
         ...
         allSwathGridCoords[0][swathIdx[0]] = g0;
         allSwathGridCoords[1][swathIdx[0]] = g1;
         allSwathGridIndexs[swathIdx[0]] = k;
